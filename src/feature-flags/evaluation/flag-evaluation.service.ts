...
import { EvaluationReason, FeatureFlag, FlagEvaluationResult, FlagValueType, UserContext, } from '../interfaces';
...
export class FlagEvaluationService {
    ...
    constructor(private readonly targetingService: TargetingService, private readonly rolloutService: RolloutService, private readonly experimentationService: ExperimentationService, private readonly analyticsService: FlagAnalyticsService) { }
    /**
    ...
    evaluate(flagKey: string, userContext: UserContext): FlagEvaluationResult {
        try {
            ...
            // Targeting rules
            if (flag.targeting) {
                const matchedVariationKey = this.targetingService.evaluateTargeting(flag.targeting, userContext);
            ...
            // A/B experiment
            if (flag.experiment) {
                const experimentResult = this.experimentationService.assignVariant(flag.experiment, flagKey, userContext);
                if (experimentResult) {
                    ...
                    this.recordEvaluation(result, userContext);
                    this.analyticsService.trackImpression(experimentResult.experimentId, experimentResult.variantKey, userContext.userId, flagKey);
                    return result;
                }
            }
            // Gradual rollout
    ...
    updateFlag(flagKey: string, updates: Partial<Omit<FeatureFlag, 'key' | 'id'>>): FeatureFlag | null {
    ...
    // Helpers
    // ---------------------------------------------------------------------------
    private buildResult(flag: FeatureFlag, variationKey: string, reason: EvaluationReason, ruleId?: string): FlagEvaluationResult {
