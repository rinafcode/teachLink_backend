...
export class ExperimentationService {
     ...
     */
    assignVariant(config: ExperimentConfig, flagKey: string, userContext: UserContext): ExperimentResult | null {
        ...
        if (config.endDate && now > config.endDate)
            return null;
        if (!this.isInExperimentTraffic(config, flagKey, userContext))
            return null;
     ...
     */
    private isInExperimentTraffic(config: ExperimentConfig, flagKey: string, userContext: UserContext): boolean {
        const bucketValue = this.resolveBucketAttributeValue(config.bucketByAttribute ?? 'userId', userContext);
     ...
     */
    private selectVariant(config: ExperimentConfig, flagKey: string, userContext: UserContext): ExperimentVariant | null {
