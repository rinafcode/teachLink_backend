...
export class AuditLogService {
    ...
    private readonly auditRepo: Repository<AuditLog>, private readonly configService: ConfigService) {
     ...
     */
    async logAuth(action: AuditAction, userId: string | null, userEmail: string | null, ipAddress: string, userAgent: string, metadata?: Record<string, unknown>, severity: AuditSeverity = AuditSeverity.INFO): Promise<AuditLog> {
        return this.log({
     ...
     */
    async logDataChange(action: AuditAction, userId: string, userEmail: string, entityType: string, entityId: string, oldValues: Record<string, unknown>, newValues: Record<string, unknown>, ipAddress?: string, description?: string): Promise<AuditLog> {
        return this.log({
            userId,
            userEmail,
            action,
     ...
     * Log API access
     */
    async logApiAccess(userId: string | null, userEmail: string | null, apiEndpoint: string, httpMethod: string, statusCode: number, responseTimeMs: number, ipAddress: string, userAgent: string, requestId?: string): Promise<AuditLog> {
     ...
     * Log security event
     */
    async logSecurityEvent(action: AuditAction, userId: string | null, userEmail: string | null, ipAddress: string, userAgent: string, description: string, metadata?: Record<string, unknown>): Promise<AuditLog> {
        return this.log({
            userId: userId || undefined,
            userEmail: userEmail || undefined,
            action,
     ...
     */
    async search(filters: AuditLogSearchFilters, page: number = 1, limit: number = 50): Promise<AuditLogSearchResult> {
    ...
    async getStatistics(): Promise<{
        ...
        const [totalLogs, logsToday, logsThisWeek, logsThisMonth, criticalEvents, errorEvents] = await Promise.all([
