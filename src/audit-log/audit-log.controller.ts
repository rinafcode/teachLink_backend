...
export class AuditLogController {
    ...
    limit?: number) {
        ...
        if (actions)
            filters.actions = actions.split(',') as AuditAction[];
        if (categories)
            filters.categories = categories.split(',') as AuditCategory[];
        if (severities)
            filters.severities = severities.split(',') as AuditSeverity[];
        if (entityType)
        ...
        if (apiEndpoint)
            filters.apiEndpoint = apiEndpoint;
        if (httpMethod)
