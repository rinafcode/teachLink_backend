...
export class AuditLogInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
        ...
        return next.handle().pipe(tap({
            next: () => {
                this.logRequest(userId, userEmail, path, method, response.statusCode, Date.now() - startTime, ip, userAgent, requestId);
            },
            error: (error) => {
                const statusCode = error.status || 500;
                this.logRequest(userId, userEmail, path, method, statusCode, Date.now() - startTime, ip, userAgent, requestId, error.message);
            },
        }));
    }
    private async logRequest(userId: string | null, userEmail: string | null, apiEndpoint: string, httpMethod: string, statusCode: number, responseTimeMs: number, ipAddress: string, userAgent: string, requestId?: string, errorMessage?: string): Promise<void> {
        try {
