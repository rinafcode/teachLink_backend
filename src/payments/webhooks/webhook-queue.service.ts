...
export class WebhookQueueService {
    ...
    async queueWebhook(payload: WebhookQueuePayload): Promise<string> {
        try {
            ...
            this.logger.log(`Webhook queued for processing. Retry ID: ${webhookRetry.id}, Job ID: ${job.id}`);
    ...
    async requeueDeadLetterWebhook(webhookRetryId: string): Promise<void> {
        try {
            ...
            if (webhookRetry.status !== WebhookStatus.DEAD_LETTER) {
                throw new Error(`Webhook is not in dead letter status. Current status: ${webhookRetry.status}`);
            }
