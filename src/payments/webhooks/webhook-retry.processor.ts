...
import { SubscriptionWebhookEvent, RefundWebhookData, } from '../interfaces/payment-provider.interface';
...
export class WebhookRetryProcessor {
    ...
    private readonly webhookRetryRepository: Repository<WebhookRetry>, private readonly providerFactory: ProviderFactoryService, private readonly paymentsService: PaymentsService) { }
    @Process(JOB_NAMES.PROCESS_WEBHOOK)
    async processWebhook(job: Job<WebhookJobData>) {
        const { webhookRetryId, provider, payload, signature, externalEventId: _externalEventId, headers, } = job.data;
        try {
    ...
    private async handleWebhookError(webhookRetryId: string, error: Error, job: Job<WebhookJobData>): Promise<void> {
        ...
        if (webhookRetry.retryCount < webhookRetry.maxRetries) {
            ...
            await this.webhookRetryRepository.save(webhookRetry);
            this.logger.warn(`Webhook ${webhookRetryId} will be retried at ${webhookRetry.nextRetryTime}. Retry ${webhookRetry.retryCount}/${webhookRetry.maxRetries}`);
            ...
            await this.webhookRetryRepository.save(webhookRetry);
            this.logger.error(`Webhook ${webhookRetryId} moved to dead letter after ${webhookRetry.retryCount} attempts. Error: ${error.message}`);
    ...
    private async handleStripeWebhook(payload: Buffer, signature: string, _webhookRetryId: string): Promise<void> {
        if (!payload) {
    ...
    private async handlePayPalWebhook(payload: Record<string, unknown>, _headers: Record<string, string>, _webhookRetryId: string): Promise<void> {
