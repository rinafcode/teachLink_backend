...
describe('PaymentsController', () => {
    ...
    it('returns validation failure for invalid refund request', async () => {
        paymentsService.processRefund.mockRejectedValue(new BadRequestException('Invalid refund amount'));
        await expectValidationFailure(() => controller.processRefund({ paymentId: 'payment-1', amount: -1, reason: 'duplicate' }));
    });
    ...
    it('returns unauthorized when access token is invalid', async () => {
        paymentsService.createPaymentIntent.mockRejectedValue(new UnauthorizedException('Invalid token'));
