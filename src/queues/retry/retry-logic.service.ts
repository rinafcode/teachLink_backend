...
export class RetryLogicService {
  ...
  calculateBackoffDelay(attemptNumber: number, strategy: IRetryStrategy): number {
    ...
    shouldRetry(error: Error, attemptNumber: number, maxAttempts: number): boolean {
        ...
        const isRetryable = retryableErrors.some((type) => error.name.includes(type) || error.message.includes(type));
        if (isRetryable) {
            this.logger.log(`Retryable error detected: ${error.name}, will retry (attempt ${attemptNumber}/${maxAttempts})`);
        }
        return isRetryable;
    }
    /**
     ...
     * Log retry attempt
     */
    logRetryAttempt(jobId: string, jobName: string, attemptNumber: number, error: Error, nextRetryDelay: number): void {
        this.logger.warn(`Job ${jobName} (${jobId}) failed on attempt ${attemptNumber}. ` +
            `Error: ${error.message}. ` +
    ...
    handleFinalFailure(jobId: string, jobName: string, error: Error, attempts: number): void {
        this.logger.error(`Job ${jobName} (${jobId}) permanently failed after ${attempts} attempts. ` +
