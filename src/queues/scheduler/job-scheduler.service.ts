...
export class JobSchedulerService {
     ...
     */
    async scheduleDelayedJob<T = unknown>(name: string, data: T, delayMs: number, options?: JobOptions): Promise<string> {
        const job = await this.defaultQueue.add(name, data, {
            ...options,
            delay: delayMs,
        });
     ...
     */
    async scheduleBatchJobs<T = unknown>(jobs: Array<{
        name: string;
        data: T;
        scheduledTime: Date;
        options?: JobOptions;
     ...
     */
    async scheduleWithBusinessHours<T = unknown>(name: string, data: T, options?: JobOptions): Promise<string> {
        const now = new Date();
