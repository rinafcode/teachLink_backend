...
export class QueueMonitoringService {
    ...
    async retryAllFailedJobs(): Promise<BulkRetryResult> {
        const failed = await this.defaultQueue.getFailed(0, 10000);
    ...
    async getRetryAnalytics(windowMinutes: number = 60): Promise<RetryAnalytics> {
        ...
        const [failedJobs, completedJobs] = await Promise.all([
            this.defaultQueue.getFailed(0, 2000),
            this.defaultQueue.getCompleted(0, 2000),
        ]);
        // Filter to window
    ...
    private async recoverStuckJobs(jobs: Job[]): Promise<void> {
        for (const job of jobs) {
            try {
                ...
                await job.moveToFailed({ message: 'Automatically moved: job exceeded stuck threshold' }, true);
            }
            catch (err) {
    ...
    private async sendAlert(health: QueueHealthStatus): Promise<void> {
        this.logger.error('QUEUE_ALERT', JSON.stringify({
    ...
    private escalate(_current: 'healthy' | 'warning' | 'critical', _next: 'warning' | 'critical'): void { }
}
