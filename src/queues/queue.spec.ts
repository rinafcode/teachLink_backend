...
class QueueMonitoringService {
  ...
  async getFailedJobs(limit = 50, offset = 0, jobName?: string) {
    if (jobName) {
      const all = await this.queue.getFailed(0, 5_000);
      const filtered = all.filter((j: any) => j.name === jobName);
  ...
  async getStuckJobs(thresholdMs = 300_000) {
    const active = await this.queue.getActive();
    const now = Date.now();
    return active.filter((job: any) => {
  ...
  async getRetryAnalytics(windowMinutes = 60) {
    ...
    const [failedJobs, completedJobs] = await Promise.all([
      this.queue.getFailed(0, 2_000),
      this.queue.getCompleted(0, 2_000),
    ]);

    const recentFailed = failedJobs.filter((j: any) => (j.finishedOn ?? j.timestamp) >= cutoff);
    const recentCompleted = completedJobs.filter(
      (j: any) => (j.finishedOn ?? j.timestamp) >= cutoff,
    );

    const successAfterRetry = recentCompleted.filter((j: any) => j.attemptsMade > 1);
    const permanentlyFailed = recentFailed.filter(
      (j: any) => j.attemptsMade >= (j.opts?.attempts ?? 3),
    );
    const retried = recentFailed.filter((j: any) => j.attemptsMade > 1);

    const byJobType: Record<string, { failed: number; retried: number; avgAttempts: number }> = {};
  ...
  private async calculateAvgProcessingTime(): Promise<number> {
    try {
      ...
      const times = completed
        .filter((j: any) => j.finishedOn != null && j.processedOn != null)
...
class QueueController {
  constructor(
    ...
    private readonly prioritizationService: any,
  ...
  async getRetryAnalytics(query: any) {
  ...
  async getJob(id: string) {
    ...
    if (!metrics) throw new Error(`Job ${id} not found`);
    return metrics;
  }
  ...
  async addBulkJobs(body: any) {
    const jobs = await this.queueService.addBulkJobs(body.jobs);
    return { count: jobs.length, jobIds: jobs.map((j: any) => j.id) };
  }

  async scheduleJob(body: any) {
    const scheduledTime = new Date(body.scheduledTime);
    if (scheduledTime <= new Date()) throw new Error('scheduledTime must be in the future');
  ...
  async scheduleDelayedJob(body: any) {
...
function buildQueue(overrides: any = {}) {
  return {
...
function buildQueueService(overrides: any = {}) {
  return {
    addJob: jest.fn(),
    addBulkJobs: jest.fn(),
    getJob: jest.fn(),
...
function buildSchedulerService(overrides: any = {}) {
  return {
    scheduleJob: jest.fn(),
...
describe('QueueMonitoringService', () => {
  ...
  describe('getQueueMetrics', () => {
    ...
    it('caps history at 100 entries', async () => {
      for (let i = 0; i < 110; i++) await service.getQueueMetrics();
  ...
  describe('throughput', () => {
    ...
    it('computes jobs/min correctly between snapshots', async () => {
      ...
      (service as any).metricsHistory[0].capturedAt = Date.now() - 60_000;
  ...
  describe('getFailedJobs', () => {
    ...
    it('filters by jobName when provided', async () => {
      ...
      expect(results.every((r: any) => r.name === 'email')).toBe(true);
    });
  ...
  describe('getStuckJobs', () => {
    it('returns jobs active longer than threshold', async () => {
      ...
      queue.getActive.mockResolvedValue([
        { id: '1', processedOn: now - 400_000 }, // stuck
        { id: '2', processedOn: now - 100_000 }, // fine
        { id: '3', processedOn: now - 600_000 }, // stuck
      ]);
      const stuck = await service.getStuckJobs(300_000);
      expect(stuck).toHaveLength(2);
      const ids = stuck.map((j: any) => j.id);
      expect(ids).toContain('1');
      expect(ids).toContain('3');
    });

    it('returns empty array when all active jobs are within threshold', async () => {
      queue.getActive.mockResolvedValue([{ id: '1', processedOn: Date.now() - 1_000 }]);
      const stuck = await service.getStuckJobs(300_000);
    ...
    it('falls back to job.timestamp when processedOn is null', async () => {
      const now = Date.now();
      queue.getActive.mockResolvedValue([
        { id: '99', processedOn: null, timestamp: now - 999_000 },
      ]);
      const stuck = await service.getStuckJobs(300_000);
  ...
  describe('getQueueStatistics', () => {
    ...
    it('trend is up when completed counts rise', async () => {
      ...
      const history = (service as any).metricsHistory as any[];
      for (let i = 0; i < 11; i++) {
        history.push({
          queueName: 'default',
          completed: i * 100,
          failed: 0,
          throughput: 0,
          capturedAt: Date.now() - (10 - i) * 60_000,
          waiting: 0,
          active: 0,
          delayed: 0,
          paused: 0,
...
describe('QueueController', () => {
      ...
      queue.getActive.mockResolvedValue([
        {
          ...
          name: 'backup',
          data: {},
          processedOn: now - 400_000,
          attemptsMade: 1,
          opts: { priority: 3 },
        },
      ]);
      const result = await controller.getStuckJobs({ threshold: 300_000 });
      expect(result.data).toHaveLength(1);
  ...
  describe('scheduleJob', () => {
    it('throws for a past scheduledTime', async () => {
      ...
      await expect(
        controller.scheduleJob({ name: 'x', data: {}, scheduledTime: past }),
      ).rejects.toThrow();
    });
  ...
  describe('cleanQueue', () => {
    it('delegates grace and status to queueService', async () => {
      ...
      await controller.cleanQueue({ grace: 10_000, status: 'failed' });
      expect(queueService.cleanQueue).toHaveBeenCalledWith(10_000, 'failed');
    });
