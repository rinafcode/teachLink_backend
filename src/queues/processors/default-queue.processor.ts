...
export class DefaultQueueProcessor {
    ...
    @Process('*')
    async handleJob(job: Job): Promise<unknown> {
     ...
     */
    private async processJobByType(job: Job): Promise<unknown> {
        switch (job.name) {
            case 'send-email':
    ...
    private async processSendEmail(job: Job): Promise<unknown> {
    ...
    private async processGenerateReport(job: Job): Promise<unknown> {
    ...
    private async processPayment(job: Job): Promise<unknown> {
    ...
    private async processBackup(job: Job): Promise<unknown> {
    ...
    @OnQueueActive()
    onActive(job: Job) {
        this.logger.log(`Job ${job.name} (${job.id}) started processing - Priority: ${job.opts.priority}`);
    }
    @OnQueueCompleted()
    onCompleted(job: Job, result: unknown) {
        const processingTime = (job.finishedOn ?? Date.now()) - (job.processedOn ?? Date.now());
        this.logger.log(`Job ${job.name} (${job.id}) completed in ${processingTime}ms - Result: ${JSON.stringify(sanitizePii(result))}`);
    }
    @OnQueueFailed()
    async onFailed(job: Job, error: Error) {
        const strategy = this.retryLogicService.getDefaultStrategy(job.name);
        const shouldRetry = this.retryLogicService.shouldRetry(error, job.attemptsMade, strategy.maxAttempts);
        if (shouldRetry) {
            const nextDelay = this.retryLogicService.calculateBackoffDelay(job.attemptsMade + 1, strategy);
            this.retryLogicService.logRetryAttempt(job.id.toString(), job.name, job.attemptsMade, error, nextDelay);
        }
        else {
            this.retryLogicService.handleFinalFailure(job.id.toString(), job.name, error, job.attemptsMade);
        }
    }
}
