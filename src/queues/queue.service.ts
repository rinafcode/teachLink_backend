...
export class QueueService {
     ...
     */
    async addJob<T = unknown>(name: string, data: T, options?: JobOptions): Promise<Job<T>> {
        try {
            ...
            this.logger.log(`Job ${name} added to queue with ID: ${job.id}, Priority: ${options?.priority || JobPriority.NORMAL}`);
            return job;
        }
        catch (error) {
     ...
     */
    async addBulkJobs<T = unknown>(jobs: Array<{
        name: string;
        data: T;
        options?: JobOptions;
    }>): Promise<Array<Job<T>>> {
