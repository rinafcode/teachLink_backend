...
export class PrioritizationService {
    ...
    adjustPriorityByAge(currentPriority: JobPriority, createdAt: Date): JobPriority {
        ...
        if (ageInHours > 24 && currentPriority > JobPriority.CRITICAL) {
            this.logger.log(`Increasing priority for job older than 24 hours: ${currentPriority} -> ${currentPriority - 1}`);
            return (currentPriority - 1) as JobPriority;
        }
        if (ageInHours > 12 && currentPriority > JobPriority.HIGH) {
            this.logger.log(`Increasing priority for job older than 12 hours: ${currentPriority} -> ${currentPriority - 1}`);
            return (currentPriority - 1) as JobPriority;
        }
