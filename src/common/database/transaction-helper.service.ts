...
export class TransactionHelperService {
     ...
     */
    async executeInTransaction<T>(operations: Array<(manager: EntityManager) => Promise<T>>): Promise<T[]> {
        const queryRunner = this.dataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        try {
            this.logger.debug(`Starting transaction with ${operations.length} operations`);
            const results = await Promise.all(operations.map((operation) => operation(queryRunner.manager)));
     ...
     */
    async executeWithRollback<T>(operations: Array<{
