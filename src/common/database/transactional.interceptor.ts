...
export class TransactionalInterceptor implements NestInterceptor {
    ...
    constructor(private readonly reflector: Reflector, private readonly transactionService: TransactionService) { }
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<unknown>> {
        ...
        try {
            let result;
            if (options.retry) {
                result = await this.transactionService.runWithRetry(async (_manager) => {
                    // Execute the original method
                    return await next.handle().toPromise();
                }, options.maxRetries || 3, options.retryDelay || 1000);
            }
            else if (options.isolationLevel) {
                result = await this.transactionService.runWithIsolationLevel(options.isolationLevel, async (_manager) => {
                    return await next.handle().toPromise();
                });
            }
            else {
