...
export class VotingTransactionExample {
     ...
     */
    async castVote(userId: string, proposalId: string, voteType: 'for' | 'against' | 'abstain', votingPower: number): Promise<unknown> {
        return this.transactionService.runWithIsolationLevel('SERIALIZABLE', // Prevent concurrent voting issues
        async (manager) => {
            // 1. Check if user already voted
            const existingVote = await manager.query('SELECT * FROM votes WHERE user_id = $1 AND proposal_id = $2', [userId, proposalId]);
            if (existingVote && existingVote.length > 0) {
                throw new Error('User has already voted on this proposal');
            }
            // 2. Check if proposal is still active
            const proposal = await manager.query('SELECT * FROM proposals WHERE id = $1 AND status = $2 FOR UPDATE', [proposalId, 'active']);
            ...
            // 4. Record the vote
            const vote = await manager.query('INSERT INTO votes (user_id, proposal_id, vote_type, voting_power) VALUES ($1, $2, $3, $4) RETURNING *', [userId, proposalId, voteType, votingPower]);
            // 5. Update proposal vote counts
            const updateField = voteType === 'for'
                ? 'votes_for'
                : voteType === 'against'
                    ? 'votes_against'
                    : 'votes_abstain';
            await manager.query(`UPDATE proposals SET ${updateField} = ${updateField} + $1, total_votes = total_votes + $1 WHERE id = $2`, [votingPower, proposalId]);
            ...
            await manager.query('INSERT INTO activity_logs (user_id, action, entity_type, entity_id) VALUES ($1, $2, $3, $4)', [userId, 'vote_cast', 'proposal', proposalId]);
     ...
     */
    async changeVote(userId: string, proposalId: string, newVoteType: 'for' | 'against' | 'abstain'): Promise<unknown> {
        return this.transactionService.runInTransaction(async (manager) => {
            // 1. Get existing vote
            const existingVote = await manager.query('SELECT * FROM votes WHERE user_id = $1 AND proposal_id = $2 FOR UPDATE', [userId, proposalId]);
            if (!existingVote || existingVote.length === 0) {
            ...
            const proposal = await manager.query('SELECT * FROM proposals WHERE id = $1 AND status = $2 AND allow_vote_change = true FOR UPDATE', [proposalId, 'active']);
            if (!proposal || proposal.length === 0) {
                throw new Error('Proposal does not allow vote changes or is not active');
            }
            // 3. Update vote counts - remove old vote
            const oldField = oldVoteType === 'for'
                ? 'votes_for'
                : oldVoteType === 'against'
                    ? 'votes_against'
                    : 'votes_abstain';
            ...
            const newField = newVoteType === 'for'
                ? 'votes_for'
                : newVoteType === 'against'
                    ? 'votes_against'
                    : 'votes_abstain';
            ...
            await manager.query('UPDATE votes SET vote_type = $1, changed_at = NOW() WHERE user_id = $2 AND proposal_id = $3', [newVoteType, userId, proposalId]);
            // 6. Log the change
            await manager.query('INSERT INTO activity_logs (user_id, action, entity_type, entity_id, metadata) VALUES ($1, $2, $3, $4, $5)', [
                userId,
                'vote_changed',
                'proposal',
                proposalId,
     ...
     */
    async executeProposal(proposalId: string): Promise<unknown> {
        return this.transactionService.runInTransaction(async (manager) => {
            ...
            const { votes_for, votes_against, total_votes, quorum_required, approval_threshold } = proposal[0];
            ...
            const actions = await manager.query('SELECT * FROM proposal_actions WHERE proposal_id = $1 ORDER BY execution_order', [proposalId]);
            ...
            // 7. Log execution
            await manager.query('INSERT INTO activity_logs (action, entity_type, entity_id, metadata) VALUES ($1, $2, $3, $4)', [
                'proposal_executed',
                'proposal',
                proposalId,
     ...
     */
    private async executeProposalAction(manager: unknown, action: unknown): Promise<void> {
