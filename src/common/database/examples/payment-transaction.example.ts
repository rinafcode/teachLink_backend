...
export class PaymentTransactionExample {
     ...
     */
    async processPayment(userId: string, amount: number, recipientId: string): Promise<unknown> {
        return this.transactionService.runInTransaction(async (manager) => {
            // 1. Deduct from sender
            const sender = await manager.query('UPDATE users SET balance = balance - $1 WHERE id = $2 AND balance >= $1 RETURNING *', [amount, userId]);
            ...
            const payment = await manager.query('INSERT INTO payments (user_id, recipient_id, amount, status) VALUES ($1, $2, $3, $4) RETURNING *', [userId, recipientId, amount, 'completed']);
            // 4. Create transaction log
            await manager.query('INSERT INTO transaction_logs (payment_id, type, amount) VALUES ($1, $2, $3)', [payment[0].id, 'payment', amount]);
     ...
     */
    async processPaymentWithRetry(userId: string, amount: number, recipientId: string): Promise<unknown> {
        return this.transactionService.runWithRetry(async (manager) => {
     ...
     */
    async processPaymentSerializable(userId: string, amount: number, recipientId: string): Promise<unknown> {
    ...
    async refundPayment(paymentId: string): Promise<unknown> {
        return this.transactionService.runInTransaction(async (manager) => {
            ...
            // 4. Create refund log
            await manager.query('INSERT INTO transaction_logs (payment_id, type, amount) VALUES ($1, $2, $3)', [paymentId, 'refund', amount]);
     ...
     */
    private async processPaymentLogic(manager: unknown, userId: string, amount: number, recipientId: string): Promise<unknown> {
        const sender = await manager.query('UPDATE users SET balance = balance - $1 WHERE id = $2 AND balance >= $1 RETURNING *', [amount, userId]);
        if (!sender || sender.length === 0) {
            throw new Error('Insufficient balance');
        }
