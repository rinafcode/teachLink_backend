 ...
 */
export const Transactional = (options: TransactionalOptions = {}) => function (target: unknown, propertyKey: string, descriptor: PropertyDescriptor): PropertyDescriptor {
    const originalMethod = descriptor.value;
    descriptor.value = async function (...args: unknown[]) {
        const transactionService: TransactionService = this.transactionService;
        if (!transactionService) {
            throw new Error(`TransactionService not injected in ${target.constructor.name}. ` +
                'Please inject it via constructor.');
        }
        const operationName = `${target.constructor.name}.${propertyKey}`;
        try {
            return await transactionService.runWithRetry(async (_manager) => {
                return await originalMethod.apply(this, args);
            }, options.maxRetries ?? 3, options.retryDelay ?? 1000);
        }
        catch (error) {
