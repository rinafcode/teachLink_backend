...
export class TransactionService {
     ...
     */
    async runWithIsolationLevel<T>(isolationLevel: 'READ UNCOMMITTED' | 'READ COMMITTED' | 'REPEATABLE READ' | 'SERIALIZABLE', operation: (manager: EntityManager) => Promise<T>): Promise<T> {
        const queryRunner = this.dataSource.createQueryRunner();
        await queryRunner.connect();
        try {
     ...
     */
    async runWithRetry<T>(operation: (manager: EntityManager) => Promise<T>, maxRetries: number = 3, retryDelay: number = 1000): Promise<T> {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (this.isRetryableError(error) && attempt < maxRetries) {
                    this.logger.warn(`Transaction failed (attempt ${attempt}/${maxRetries}), retrying in ${retryDelay}ms...`);
     ...
     */
    async runParallelInTransaction<T>(operations: Array<(manager: EntityManager) => Promise<T>>): Promise<T[]> {
     ...
     */
    async runWithSavepoint<T>(savepointName: string, operation: (manager: EntityManager) => Promise<T>, parentManager?: EntityManager): Promise<T> {
        if (parentManager) {
