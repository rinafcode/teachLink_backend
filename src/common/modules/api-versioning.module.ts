...
import { ApiVersionInterceptor, API_VERSION_HEADER, API_VERSION_HEADER_KEY, DEFAULT_API_VERSION, isVersionNeutralPath, normalizeRequestedApiVersion, SUPPORTED_API_VERSIONS, VersionedRequest, } from '../interceptors/api-version.interceptor';
...
export class ApiVersionValidationMiddleware implements NestMiddleware {
    use(req: Request & VersionedRequest & {
        ...
        if (!rawVersion) {
            throw new BadRequestException(`Missing required ${API_VERSION_HEADER} header. Supported versions: ${SUPPORTED_API_VERSIONS.join(', ')}`);
        }
        const normalizedVersion = normalizeRequestedApiVersion(rawVersion);
        if (!normalizedVersion) {
            throw new BadRequestException(`Invalid ${API_VERSION_HEADER} header value "${Array.isArray(rawVersion) ? rawVersion[0] : rawVersion}". Expected values like "1" or "v1".`);
        }
        if (!SUPPORTED_API_VERSIONS.includes(normalizedVersion)) {
            throw new NotAcceptableException(`Unsupported API version "${normalizedVersion}". Supported versions: ${SUPPORTED_API_VERSIONS.join(', ')}`);
        }
