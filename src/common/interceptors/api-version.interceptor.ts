...
export const DEFAULT_API_VERSION = normalizeConfiguredVersion(process.env.API_DEFAULT_VERSION?.trim() || '1');
...
export function isVersionNeutralPath(pathOrUrl: string): boolean {
    ...
    return VERSION_NEUTRAL_PATH_PREFIXES.some((prefix) => path === prefix || path.startsWith(`${prefix}/`));
}
@Injectable()
