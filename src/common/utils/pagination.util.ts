 ...
 */
export async function paginateWithCursor<T extends Record<string, unknown>>(queryBuilder: SelectQueryBuilder<T>, options: CursorPaginationQueryDto): Promise<CursorPaginatedResponse<T>> {
    const limit = options.limit || DEFAULT_PAGE_SIZE;
    ...
    if (options.cursor) {
        ...
        queryBuilder.andWhere(`(${alias}.${sortBy} ${op} :sortValue` +
            ` OR (${alias}.${sortBy} = :sortValue AND ${alias}.id ${op} :cursorId))`, { sortValue, cursorId });
    }
    ...
    const nextCursor = pageData.length > 0 && (isForward ? hasMore : !!options.cursor)
    ...
    const prevCursor = pageData.length > 0 && (!isForward ? hasMore : !!options.cursor)
