import { correlationMiddleware, getCorrelationId, injectCorrelationIdToHeaders, CORRELATION_ID_HEADER, } from './correlation.utils';
describe('correlation.utils', () => {
    it('generates and propagates correlation ID through middleware', (done) => {
        const req: unknown = { method: 'GET', url: '/test', headers: {} };
    ...
    it('respects incoming x-request-id header', (done) => {
        ...
        const req: unknown = { method: 'GET', url: '/test', headers: { 'x-request-id': incomingId } };
        const headers: Record<string, string> = {};
        const res: unknown = {
            setHeader: (name: string, value: string) => {
                headers[name.toLowerCase()] = value;
