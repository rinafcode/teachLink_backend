...
export class DisasterRecoveryService {
    ...
    constructor(private readonly backupService: BackupService, private readonly alertingService: AlertingService, private readonly fileStorageService: FileStorageService, private readonly configService: ConfigService) {
    ...
    async executeRestore(backupId: string): Promise<void> {
        ...
        try {
            ...
            const backupData = await this.fileStorageService.downloadFile(backup.replicatedStorageKey || backup.encryptedStorageKey);
            ...
            if (rtoActual > RTO_THRESHOLD_SECONDS) {
                this.alertingService.sendAlert('DISASTER_RECOVERY_RTO_EXCEEDED', `Disaster recovery completed but RTO exceeded: ${rtoActual}s > ${RTO_THRESHOLD_SECONDS}s`, 'CRITICAL');
            }
            else {
                this.alertingService.sendAlert('DISASTER_RECOVERY_SUCCESS', `Disaster recovery completed successfully in ${rtoActual} seconds`, 'INFO');
            }
        }
        catch (error) {
            this.logger.error('Disaster recovery failed:', error);
            this.alertingService.sendAlert('DISASTER_RECOVERY_FAILED', `Disaster recovery failed: ${error.message}`, 'CRITICAL');
            throw error;
        }
    }
