...
import { ScheduledTaskConfig, ScheduledTaskMonitoringService, } from '../monitoring/scheduled-task-monitoring.service';
@Injectable()
export class BackupService {
    ...
    private readonly backupQueue: Queue, private readonly configService: ConfigService, private readonly alertingService: AlertingService, private readonly metricsService: MetricsCollectionService, private readonly scheduledTaskMonitoringService: ScheduledTaskMonitoringService) {
        this.retentionDays = this.configService.get<number>('BACKUP_RETENTION_DAYS', 30);
        this.scheduledTaskRetryLimit = this.configService.get<number>('BACKUP_SCHEDULED_TASK_RETRY_LIMIT', 2);
        this.scheduledTaskRetryDelayMs = this.configService.get<number>('BACKUP_SCHEDULED_TASK_RETRY_DELAY_MS', 10000);
        this.scheduledTaskTimeoutMs = this.configService.get<number>('BACKUP_SCHEDULED_TASK_TIMEOUT_MS', 30 * 60 * 1000);
    ...
    async handleScheduledBackup(): Promise<void> {
        await this.executeMonitoredScheduledTask('weekly-database-backup', {
            expectedIntervalMs: 7 * 24 * 60 * 60 * 1000,
            timeoutMs: this.scheduledTaskTimeoutMs,
            maxRetries: this.scheduledTaskRetryLimit,
        }, async () => {
            this.logger.log('Starting scheduled weekly backup');
            const region = (this.configService.get<string>('BACKUP_PRIMARY_REGION') as Region) || Region.US_EAST_1;
    ...
    async handleBackupCleanup(): Promise<void> {
        await this.executeMonitoredScheduledTask('cleanup-expired-backups', {
            expectedIntervalMs: 24 * 60 * 60 * 1000,
            timeoutMs: this.scheduledTaskTimeoutMs,
            maxRetries: this.scheduledTaskRetryLimit,
        }, async () => {
            ...
            for (const backup of expiredBackups) {
                await this.backupQueue.add(JOB_NAMES.DELETE_BACKUP, { backupRecordId: backup.id }, {
                    attempts: 3,
    ...
    private async executeMonitoredScheduledTask(taskName: string, config: ScheduledTaskConfig, taskRunner: () => Promise<void>): Promise<void> {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            try {
                ...
                this.logger.error(`Scheduled task ${taskName} failed on attempt ${attempt}/${maxAttempts}: ${errorMessage}`, error instanceof Error ? error.stack : undefined);
                if (shouldRetry) {
                ...
                this.alertingService.sendAlert('BACKUP_SCHEDULED_FAILED', `Scheduled task ${taskName} failed after ${maxAttempts} attempt(s): ${errorMessage}`, 'CRITICAL');
            }
        }
    }
