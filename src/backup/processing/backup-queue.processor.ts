...
import { BackupJobData, VerificationJobData, RecoveryTestJobData, } from '../interfaces/backup.interfaces';
...
export class BackupQueueProcessor {
    ...
    private readonly backupRepository: Repository<BackupRecord>, private readonly backupService: BackupService, private readonly fileStorageService: FileStorageService, private readonly dataIntegrityService: DataIntegrityService, private readonly configService: ConfigService) {
    ...
    async handleCreateBackup(job: Job<BackupJobData>) {
        ...
        try {
            ...
            const secondaryRegion = this.configService.get<string>('BACKUP_SECONDARY_REGION', 'us-west-2');
            ...
            await (job.queue as unknown).add(JOB_NAMES.VERIFY_BACKUP, { backupRecordId, storageKey: encryptedKey }, {
                attempts: 3,
    ...
    async handleDeleteBackup(job: Job<{
        ...
        try {
            ...
            if (backup.replicatedStorageKey) {
                const secondaryBucket = this.configService.get<string>('AWS_S3_BUCKET_SECONDARY', bucketName);
                const secondaryS3Client = new S3Client({
                    region: this.configService.get<string>('BACKUP_SECONDARY_REGION', 'us-west-2'),
                });
                await secondaryS3Client.send(new DeleteObjectCommand({
    ...
    private async encryptBackup(storageKey: string, kmsKeyId: string): Promise<string> {
        ...
        await this.fileStorageService.uploadProcessedFile(Buffer.from(response.CiphertextBlob), encryptedKey, 'application/octet-stream');
        return encryptedKey;
    }
    ...
    private async handleProcessingError(backup: BackupRecord, error: Error, attemptsMade: number): Promise<void> {
