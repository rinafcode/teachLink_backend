...
export class DataIntegrityService {
    ...
    private readonly backupRepository: Repository<BackupRecord>, private readonly fileStorageService: FileStorageService) { }
    async verifyBackupIntegrity(backupId: string): Promise<boolean> {
        ...
        try {
            ...
            if (md5Match && sha256Match) {
                ...
                this.logger.error(`Backup ${backupId} integrity verification failed. MD5: ${md5Match}, SHA256: ${sha256Match}`);
                return false;
            }
        }
        catch (error) {
