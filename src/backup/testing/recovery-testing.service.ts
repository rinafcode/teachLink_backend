...
export class RecoveryTestingService {
  ...
  async executeRecoveryTest(testId: string): Promise<void> {
    ...
    async executeRecoveryTest(testId: string): Promise<void> {
        ...
        try {
            ...
            const backupData = await this.fileStorageService.downloadFile(test.backupRecord.encryptedStorageKey);
            ...
            await this.recoveryTestRepository.save(test);
            // Send alert
            this.alertingService.sendAlert('RECOVERY_TEST_COMPLETED', `Recovery test ${testId} ${test.status}`, test.status === RecoveryTestStatus.PASSED ? 'INFO' : 'CRITICAL');
        }
        catch (error) {
  ...
  async getTestResults(testId: string): Promise<RecoveryTestResponseDto> {
    ...
    private async validateRestoredDatabase(dbName: string): Promise<unknown> {
        const client = new Client({
            host: this.configService.get<string>('DB_HOST', 'localhost'),
            port: parseInt(this.configService.get<string>('DB_PORT', '5432')),
            user: this.configService.get<string>('DB_USERNAME', 'postgres'),
