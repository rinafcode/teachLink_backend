...
describe('BackupService', () => {
  ...
  let mockBackupQueue: jest.Mocked<any>;
  let mockConfigService: jest.Mocked<any>;
  let mockAlertingService: jest.Mocked<any>;
  let mockMetricsService: jest.Mocked<any>;
  let mockScheduledTaskMonitoringService: jest.Mocked<any>;
  ...
  describe('constructor', () => {
    it('should initialize with default configuration values', () => {
      ...
      const defaultService = new (BackupService as any)(
        mockBackupRepo,
        mockBackupQueue,
      ...
      expect((defaultService as any).retentionDays).toBe(30);
      expect((defaultService as any).scheduledTaskRetryLimit).toBe(2);
      expect((defaultService as any).scheduledTaskRetryDelayMs).toBe(10000);
      expect((defaultService as any).scheduledTaskTimeoutMs).toBe(30 * 60 * 1000);
    });

    it('should register scheduled tasks', () => {
      expect(mockScheduledTaskMonitoringService.registerTask).toHaveBeenCalledWith(
  ...
  describe('handleScheduledBackup', () => {
    ...
    beforeEach(() => {
      ...
      mockBackupQueue.add.mockResolvedValue({} as any);
    ...
    it('should create and queue a scheduled backup', async () => {
      ...
      expect(mockScheduledTaskMonitoringService.startExecution).toHaveBeenCalledWith(
        'weekly-database-backup',
        {
          expectedIntervalMs: 7 * 24 * 60 * 60 * 1000,
          timeoutMs: 30 * 60 * 1000,
      ...
      expect(mockBackupRepo.create).toHaveBeenCalledWith(
    ...
    it('should handle complete failure after retries', async () => {
      ...
      expect(mockAlertingService.sendAlert).toHaveBeenCalledWith(
        ...
        'Scheduled task weekly-database-backup failed after 3 attempt(s): Persistent error',
        'CRITICAL',
      );
    });
  });
  ...
  describe('updateBackupStatus', () => {
    ...
    it('should update backup status to failed and send critical alert', async () => {
      ...
      expect(mockAlertingService.sendAlert).toHaveBeenCalledWith(
        'BACKUP_FAILED',
        `Backup ${backupId} failed: Test error`,
        'CRITICAL',
      );
    });
  ...
  describe('private methods', () => {
    describe('executeMonitoredScheduledTask', () => {
      ...
      it('should fail after all retries exhausted', async () => {
        ...
        expect(mockAlertingService.sendAlert).toHaveBeenCalledWith(
          'BACKUP_SCHEDULED_FAILED',
          'Scheduled task test-task failed after 3 attempt(s): Persistent failure',
          'CRITICAL',
        );
      });
    });
