...
import { ScalingOperationException, ContainerNotFoundException, DeploymentFailedException } from '../../common/exceptions/custom-exceptions';
...
describe('AutoScalingService', () => {
  ...
  describe('scaleUp', () => {
    it('should successfully scale up a container', async () => {
      ...
      containerRepository.save.mockResolvedValue({ ...mockContainerWithReplicas, replicas: 3 });
      scalingQueue.add.mockResolvedValue({ id: 'job-id' });

      await service.scaleUp(containerId);

      expect(containerRepository.findOne).toHaveBeenCalledWith({ where: { id: containerId } });
      expect(containerRepository.save).toHaveBeenCalledWith({ ...mockContainerWithReplicas, replicas: 3 });
      expect(scalingQueue.add).toHaveBeenCalledWith('scale-up', { containerId });
      expect(metricsService.recordCustomMetric).toHaveBeenCalledWith('scale_up_operations', 1);
    });

    it('should throw ScalingOperationException when containerId is empty', async () => {
      await expect(service.scaleUp('')).rejects.toThrow(ScalingOperationException);
    });
    ...
    it('should throw ContainerNotFoundException when container not found', async () => {
      ...
      await expect(service.scaleUp('non-existent-id')).rejects.toThrow(ContainerNotFoundException);
    });
    ...
    it('should throw ScalingOperationException when maximum replicas reached', async () => {
      ...
      await expect(service.scaleUp('test-container-id')).rejects.toThrow(ScalingOperationException);
    });
    ...
    it('should throw DeploymentFailedException when database save fails', async () => {
      ...
      await expect(service.scaleUp('test-container-id')).rejects.toThrow(DeploymentFailedException);
    });
    ...
    it('should record error metrics when scaling fails', async () => {
      ...
      expect(metricsService.recordCustomMetric).toHaveBeenCalledWith('scaling_operation_failures', 1);
      expect(metricsService.recordCustomMetric).toHaveBeenCalledWith('scale_up_failures', 1);
    });
  });
  ...
  describe('scaleDown', () => {
    it('should successfully scale down a container', async () => {
      ...
      containerRepository.findOne.mockResolvedValue(mockContainerWithReplicas);
      containerRepository.save.mockResolvedValue({ ...mockContainerWithReplicas, replicas: 2 });
      scalingQueue.add.mockResolvedValue({ id: 'job-id' });

      await service.scaleDown(containerId);

      expect(containerRepository.findOne).toHaveBeenCalledWith({ where: { id: containerId } });
      expect(containerRepository.save).toHaveBeenCalledWith({ ...mockContainerWithReplicas, replicas: 2 });
      expect(scalingQueue.add).toHaveBeenCalledWith('scale-down', { containerId });
      expect(metricsService.recordCustomMetric).toHaveBeenCalledWith('scale_down_operations', 1);
    });

    it('should throw ScalingOperationException when containerId is empty', async () => {
      await expect(service.scaleDown('')).rejects.toThrow(ScalingOperationException);
    });

    it('should throw ContainerNotFoundException when container not found', async () => {
      containerRepository.findOne.mockResolvedValue(null);

      await expect(service.scaleDown('non-existent-id')).rejects.toThrow(ContainerNotFoundException);
    });
    ...
    it('should throw ScalingOperationException when replicas is 1', async () => {
      ...
      await expect(service.scaleDown('test-container-id')).rejects.toThrow(ScalingOperationException);
    });
    ...
    it('should record error metrics when scaling down fails', async () => {
      ...
      expect(metricsService.recordCustomMetric).toHaveBeenCalledWith('scaling_operation_failures', 1);
      expect(metricsService.recordCustomMetric).toHaveBeenCalledWith('scale_down_failures', 1);
    });
  });
});
