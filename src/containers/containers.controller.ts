...
export class ContainersController {
  ...
  async checkHealth(@Param('containerId') containerId: string) {
    const health =
      await this.monitoringService.checkContainerHealth(containerId);
    return {
      containerId,
  ...
  @ApiParam({
    name: 'containerId',
    description: 'Container ID to get metrics for',
  })
  @ApiQuery({
    name: 'period',
    required: false,
  ...
  async getMetrics(
    ...
    const metrics =
      await this.monitoringService.getContainerMetrics(containerId);
    return {
      containerId,
      ...metrics,
      period,
