...
import { BulkDeleteMediaDto } from '../../src/media/dto/media.dto';
...
describe('BulkDeleteMediaDto', () => {
    ...
    it('rejects an empty array', async () => {
        const errors = await validate(
            plainToInstance(BulkDeleteMediaDto, { contentIds: [] }),
        );
        expect(errors.some((e) => e.property === 'contentIds')).toBe(true);
    });
    ...
    it('rejects an array containing non-string values', async () => {
        const errors = await validate(
            plainToInstance(BulkDeleteMediaDto, { contentIds: [123, 456] }),
        );
        expect(errors.some((e) => e.property === 'contentIds')).toBe(true);
    });
