...
import { ALLOWED_FILE_TYPES, ALLOWED_EXTENSIONS, FILE_SIZE_LIMITS, IMAGE_DIMENSION_LIMITS, MAGIC_NUMBERS, } from './file-validation.constants';
...
export class FileValidationService {
     ...
     */
    private async validateFileSignature(file: UploadedFile): Promise<{
        valid: boolean;
        detectedType?: string;
    }> {
    ...
    private getFileType(mimeType: string): 'image' | 'video' | 'document' | 'audio' | 'archive' | 'unknown' {
        ...
        if (ALLOWED_FILE_TYPES.IMAGES.includes(normalizedMime as (typeof ALLOWED_FILE_TYPES.IMAGES)[number]))
            return 'image';
        if (ALLOWED_FILE_TYPES.VIDEOS.includes(normalizedMime as (typeof ALLOWED_FILE_TYPES.VIDEOS)[number]))
            return 'video';
        if (ALLOWED_FILE_TYPES.DOCUMENTS.includes(normalizedMime as (typeof ALLOWED_FILE_TYPES.DOCUMENTS)[number]))
            return 'document';
        if (ALLOWED_FILE_TYPES.AUDIO.includes(normalizedMime as (typeof ALLOWED_FILE_TYPES.AUDIO)[number]))
            return 'audio';
        if (ALLOWED_FILE_TYPES.ARCHIVES.includes(normalizedMime as (typeof ALLOWED_FILE_TYPES.ARCHIVES)[number]))
            return 'archive';
        return 'unknown';
    }
    /**
     ...
     */
    private validateFileSize(size: number, fileType: string): {
        valid: boolean;
        maxSize: number;
    } {
     ...
     */
    private async checkSecurityPatterns(file: UploadedFile): Promise<{
        valid: boolean;
        issues: string[];
    }> {
        const issues: string[] = [];
