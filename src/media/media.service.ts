...
export class MediaService {
    ...
    private readonly contentRepo: Repository<ContentMetadata>, private readonly storage: FileStorageService, private readonly videoProcessing: VideoProcessingService, private readonly fileValidation: FileValidationService, private readonly malwareScanning: MalwareScanningService, private readonly imageProcessing: ImageProcessingService, private readonly uploadProgress: UploadProgressService) { }
    async createFromUpload(ownerId: string, tenantId: string | undefined, file: UploadedFile, options: UploadOptions = {}): Promise<UploadResult> {
        ...
        try {
            ...
            if (options.scanForMalware !== false) {
                if (!this.malwareScanning.isScanningAvailable()) {
                    const errorMsg = 'Malware scanning is required for uploads but no scanning service is available';
                    if (options.trackProgress !== false) {
                ...
                if (!scanResult.clean) {
                    ...
                    const errorMsg = detectedThreats.length > 0
            ...
            if (options.trackProgress !== false) {
                await this.uploadProgress.markFailed(uploadId, error instanceof Error ? error.message : 'Unknown error');
            }
            throw error;
        }
    }
