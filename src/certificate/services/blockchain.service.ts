...
export class BlockchainService {
   ...
   */
  async createCertificateOnBlockchain(
    certificateData: CertificateBlockchainData,
  ): Promise<BlockchainTransaction> {
    try {
      this.logger.log(
        `Creating certificate on blockchain: ${certificateData.certificateId}`,
      );
      ...
      const transaction: BlockchainTransaction = {
        ...
        network: this.configService.get(
          'BLOCKCHAIN_NETWORK',
          'ethereum-testnet',
        ),
        timestamp: new Date(),
      ...
      this.logger.error(
        `Failed to create certificate on blockchain: ${error.message}`,
      );
  ...
  async verifyCertificateOnBlockchain(txHash: string): Promise<{
    ...
    try {
      ...
      this.logger.error(
        `Failed to verify certificate on blockchain: ${error.message}`,
      );
      return { isValid: false };
    }
  }
   ...
   */
  async revokeCertificateOnBlockchain(
    txHash: string,
    reason: string,
  ): Promise<BlockchainTransaction> {
    try {
      ...
      this.logger.log(
        `Certificate revoked on blockchain with tx: ${revocationTxHash}`,
      );
      return transaction;
    } catch (error) {
      this.logger.error(
        `Failed to revoke certificate on blockchain: ${error.message}`,
      );
  ...
  private generateRevocationHash(
  ...
  private simulateCertificateDataRetrieval(
    txHash: string,
