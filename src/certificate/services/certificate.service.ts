...
export class CertificateService {
    ...
    try {
      ...
      try {
        const blockchainData: CertificateBlockchainData = {
          ...
          timestamp: Date.now(),
          signature:
            this.blockchainService.generateDigitalSignature(savedCertificate),
        };

        const blockchainTx =
          await this.blockchainService.createCertificateOnBlockchain(
            blockchainData,
          );
  ...
  async revoke(id: string, reason: string): Promise<CertificateResponseDto> {
    ...
    if (certificate.blockchainTxHash) {
      try {
        await this.blockchainService.revokeCertificateOnBlockchain(
    ...
    try {
      ...
      if (certificate.blockchainTxHash) {
        blockchainVerification =
          await this.blockchainService.verifyCertificateOnBlockchain(
            certificate.blockchainTxHash,
          );
      }
  ...
  async getSkillCertificates(
    ...
    const certificates = await this.certificateRepository
      ...
      .andWhere('certificate.skills @> :skill', {
  ...
  private mapToResponseDto(certificate: Certificate): CertificateResponseDto {
    const isExpired =
      certificate.expiresAt && new Date() > certificate.expiresAt;
    const isValid =
      certificate.status === CertificateStatus.ISSUED && !isExpired;
