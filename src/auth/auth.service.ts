...
export class AuthService {
    ...
    async register(registerDto: RegisterDto, ipAddress?: string, userAgent?: string): Promise<RegisterResponse> {
        return await this.transactionService.runInTransaction(async (_manager) => {
            ...
            await this.usersService.updateEmailVerificationToken(user.id, verificationToken, verificationExpires);
    ...
    async logout(userId: string, sessionId?: string, ipAddress?: string, userAgent?: string): Promise<{
        message: string;
    }> {
        ...
        // Log logout
        await this.auditLogService.logAuth(AuditAction.LOGOUT, userId, user?.email || null, ipAddress || 'unknown', userAgent || 'unknown', { sessionId });
