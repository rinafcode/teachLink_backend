...
describe('RegisterDto', () => {
    ...
    it('rejects a username longer than 30 characters', async () => {
        const errors = await validate(plainToInstance(RegisterDto, { ...valid, username: 'a'.repeat(31) }));
        expect(errors.some((e) => e.property === 'username')).toBe(true);
    });

    it('rejects a username with spaces or special characters', async () => {
        const errors = await validate(plainToInstance(RegisterDto, { ...valid, username: 'user name!' }));
        expect(errors.some((e) => e.property === 'username')).toBe(true);
    });

    it('accepts a username with underscores and hyphens', async () => {
        const errors = await validate(plainToInstance(RegisterDto, { ...valid, username: 'tech-user_1' }));
        expect(errors).toHaveLength(0);
    });
    ...
    it('rejects a password with no uppercase letter', async () => {
        const errors = await validate(plainToInstance(RegisterDto, { ...valid, password: 'secure@123' }));
        expect(errors.some((e) => e.property === 'password')).toBe(true);
    });

    it('rejects a password with no special character', async () => {
        const errors = await validate(plainToInstance(RegisterDto, { ...valid, password: 'Secure123' }));
        expect(errors.some((e) => e.property === 'password')).toBe(true);
    });

    it('rejects a password with no digit', async () => {
        const errors = await validate(plainToInstance(RegisterDto, { ...valid, password: 'Secure@abc' }));
        expect(errors.some((e) => e.property === 'password')).toBe(true);
    });
    ...
    it('rejects an invalid avatarUrl', async () => {
        const errors = await validate(plainToInstance(RegisterDto, { ...valid, avatarUrl: 'not-a-url' }));
        expect(errors.some((e) => e.property === 'avatarUrl')).toBe(true);
    });

    it('rejects a displayName longer than 60 characters', async () => {
        const errors = await validate(plainToInstance(RegisterDto, { ...valid, displayName: 'x'.repeat(61) }));
