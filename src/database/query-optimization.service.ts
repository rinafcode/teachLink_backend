...
export class QueryOptimizationService {
  ...
  analyse(sql: string): QuerySuggestion[] {
    ...
    if (q.includes('select *')) {
      suggestions.push({ issue: 'SELECT *', suggestion: 'Select only required columns to reduce data transfer.' });
    }
    if (!q.includes('where') && (q.includes('update') || q.includes('delete'))) {
      suggestions.push({ issue: 'Missing WHERE', suggestion: 'Add a WHERE clause to avoid full-table mutations.' });
    }
    if (q.includes('like') && q.match(/like\s+'%[^%]/)) {
      suggestions.push({ issue: 'Leading wildcard', suggestion: 'Avoid leading % in LIKE — it prevents index use.' });
    }
