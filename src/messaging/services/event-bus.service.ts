...
export class EventBusService {
  ...
  async publishEvent(event: Omit<Event, "id" | "timestamp" | "metadata">): Promise<string> {
    ...
    try {
      ...
      this.logger.log(`Event ${eventId} of type ${event.type} published successfully`)
  ...
  async publishBulkEvents(events: Array<Omit<Event, "id" | "timestamp" | "metadata">>): Promise<string[]> {
  ...
  subscribe(eventType: string, handler: Function, options?: { async?: boolean; filter?: Function }): void {
    ...
    try {
      ...
      const promises = Array.from(allHandlers).map(async (handler) => {
        ...
        try {
          ...
          this.logger.error(`Event handler ${handler.name} failed for event ${event.id}: ${error.message}`)
  ...
  private async processSyncHandlers(event: Event): Promise<void> {
    ...
    const syncHandlers = this.eventHandlers.get(`${event.type}:sync`) || new Set()

    for (const handler of syncHandlers) {
      try {
        await handler(event)
      } catch (error) {
        this.logger.error(`Sync event handler failed for event ${event.id}: ${error.message}`)
      }
    }
  }
