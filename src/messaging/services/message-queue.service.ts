...
export class MessageQueueService {
  ...
  async sendMessage(
    ...
    try {
      ...
      this.logger.error(
        `Failed to queue message ${messageId}: ${error.message}`,
      );
  ...
  async cancelMessage(messageId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Failed to cancel message ${messageId}: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async retryMessage(messageId: string): Promise<boolean> {
    try {
      ...
      const delay =
        Math.pow(2, messageLog.metadata.retryCount) * this.config.retryDelay;
      ...
      this.logger.log(
        `Message ${messageId} scheduled for retry (attempt ${messageLog.metadata.retryCount})`,
      );
      return true;
    } catch (error) {
      this.logger.error(
        `Failed to retry message ${messageId}: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async purgeQueue(
    queueName: 'message' | 'retry' | 'dead-letter' = 'message',
  ): Promise<void> {
  ...
  private async moveToDeadLetter(messageLog: MessageLog): Promise<void> {
    ...
    this.logger.warn(
      `Message ${messageLog.messageId} moved to dead letter queue`,
    );
  }
