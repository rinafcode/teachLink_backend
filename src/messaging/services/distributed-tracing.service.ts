...
export class DistributedTracingService {
  ...
  async searchTraces(filters: {
    ...
    if (filters.minDuration) {
      queryBuilder.andWhere('span.duration >= :minDuration', {
        minDuration: filters.minDuration,
      });
    }

    if (filters.maxDuration) {
      queryBuilder.andWhere('span.duration <= :maxDuration', {
        maxDuration: filters.maxDuration,
      });
    }

    if (filters.startTime) {
      queryBuilder.andWhere('span.startTime >= :startTime', {
        startTime: filters.startTime,
      });
    }

    if (filters.endTime) {
      queryBuilder.andWhere('span.endTime <= :endTime', {
