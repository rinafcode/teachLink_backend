...
export class ServiceDiscoveryService implements OnModuleInit, OnModuleDestroy {
  ...
  async registerService(serviceInfo?: Partial<ServiceInfo>): Promise<string> {
    ...
    this.logger.log(
      `Service ${this.serviceName} registered with ID: ${this.serviceId}`,
    );
  ...
  async discoverServices(
    ...
    const queryBuilder =
      this.serviceRegistryRepository.createQueryBuilder('service');
    ...
    if (serviceName) {
      queryBuilder.andWhere('service.serviceName = :serviceName', {
        serviceName,
      });
    }

    if (tags && tags.length > 0) {
      queryBuilder.andWhere("service.metadata->>'tags' ?| array[:...tags]", {
        tags,
      });
    }
    ...
    for (const service of services) {
      try {
        ...
        if (isHealthy) {
          ...
          const newStatus =
            newFailureCount >= service.healthCheck.retries
          ...
          if (newStatus === ServiceStatus.UNHEALTHY) {
            this.logger.warn(
              `Service ${service.serviceName} marked as unhealthy`,
            );
          }
        }
      } catch (error) {
        this.logger.error(
          `Health check failed for service ${service.serviceName}: ${error.message}`,
        );
      }
    }
  }
  ...
  async getServiceMetrics(): Promise<{
    ...
    const averageResponseTime =
      services.length > 0
        ? services.reduce((sum, s) => sum + Number(s.responseTime), 0) /
          services.length
        : 0;
