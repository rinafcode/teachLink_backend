...
export class EventBusService {
    ...
    constructor(private readonly eventEmitter: EventEmitter2, private readonly tracingService: TracingService) { }
    async publish(eventType: string, payload: unknown, source: string = 'unknown'): Promise<void> {
    ...
    async getListeners(eventType: string): Promise<unknown[]> {
        return this.eventEmitter.listeners(eventType);
    }
    async emitAsync(eventType: string, payload: unknown, source: string = 'unknown'): Promise<void> {
