...
describe('MessageQueueService', () => {
  ...
  beforeEach(async () => {
    ...
    messageLogRepository = module.get<Repository<MessageLog>>(
      getRepositoryToken(MessageLog),
    );
  });
...
describe('ServiceDiscoveryService', () => {
  ...
  describe('discoverServices', () => {
    it('should discover services by name', async () => {
      ...
      mockServiceRegistryRepository.createQueryBuilder.mockReturnValue(
        mockQueryBuilder,
      );
    ...
    it('should discover services by tags', async () => {
      const mockServices = [
        { serviceName: 'api-service', status: ServiceStatus.HEALTHY },
      ];
      ...
      const services = await service.discoverServices(undefined, [
        'api',
        'microservice',
      ]);
      ...
      expect(services).toEqual(mockServices);
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith(
        "service.metadata->>'tags' ?| array[:...tags]",
        {
...
describe('CircuitBreakerService', () => {
  ...
  beforeEach(async () => {
    ...
    circuitBreakerRepository = module.get<Repository<CircuitBreakerState>>(
      getRepositoryToken(CircuitBreakerState),
    );
  });
  ...
  describe('executeWithCircuitBreaker', () => {
    it('should execute function successfully when circuit is closed', async () => {
      ...
      expect(mockCircuitBreakerRepository.increment).toHaveBeenCalledWith(
        { id: 'cb-123' },
        'successCount',
        1,
      );
    ...
    it('should throw CircuitBreakerOpenException when circuit is open', async () => {
      ...
      mockCircuitBreakerRepository.findOne.mockResolvedValue(
        mockCircuitBreaker,
      );
      ...
      ).rejects.toThrow('Circuit breaker is open for user-service:getUser');
