...
export class MessagingController {
  ...
  async sendMessage(message: Omit<Message, 'id' | 'metadata'>) {
    if (!message.type || !message.source || !message.payload) {
      throw new BadRequestException(
        'Missing required fields: type, source, payload',
      );
    }
  ...
  async sendBulkMessages(body: {
    messages: Array<Omit<Message, 'id' | 'metadata'>>;
  }) {
    if (!body.messages || !Array.isArray(body.messages)) {
      throw new BadRequestException('Messages array is required');
    }

    const messageIds = await this.messageQueueService.sendBulkMessages(
      body.messages,
    );
  ...
  async scheduleMessage(body: {
    message: Omit<Message, 'id' | 'metadata'>;
    delay: number;
  }) {
    ...
    const messageId = await this.messageQueueService.scheduleMessage(
      body.message,
      body.delay,
    );
  ...
  async publishEvent(event: Omit<Event, 'id' | 'timestamp' | 'metadata'>) {
    if (!event.type || !event.source || !event.data) {
      throw new BadRequestException(
        'Missing required fields: type, source, data',
      );
    }
  ...
  async registerService(serviceInfo: Partial<ServiceInfo>) {
    const serviceId =
      await this.serviceDiscoveryService.registerService(serviceInfo);
  ...
  async getServiceByName(@Param('serviceName') serviceName: string) {
    const service =
      await this.serviceDiscoveryService.getServiceByName(serviceName);
    if (!service) {
  ...
  async updateServiceStatus(
    ...
    await this.serviceDiscoveryService.updateServiceStatus(
      serviceId,
      body.status,
    );
  ...
  async getCircuitBreakerState(
    ...
    const state = await this.circuitBreakerService.getCircuitBreakerState(
      serviceName,
      operation,
    );
    if (!state) {
  ...
  async resetCircuitBreaker(
    ...
    await this.circuitBreakerService.resetCircuitBreaker(
      serviceName,
      operation,
    );
    return { status: 'reset' };
  ...
  async forceOpenCircuit(
    ...
    await this.circuitBreakerService.forceOpenCircuit(
      serviceName,
      operation,
