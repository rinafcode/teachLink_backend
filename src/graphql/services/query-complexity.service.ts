...
export class QueryComplexityService {
     ...
     */
    private calculateComplexity(operation: OperationDefinitionNode, variables?: Record<string, unknown>): number {
        ...
        const visitor = {
            Field: {
                enter: (node: FieldNode): void => {
                    ...
                    let fieldComplexity = this.config.fieldComplexityMap[fieldName] || this.config.defaultFieldComplexity;
                    ...
                    if (totalComplexity > this.config.maxComplexity) {
                        ...
                        throw new Error(`Query complexity ${totalComplexity} exceeds maximum allowed complexity of ${this.config.maxComplexity}`);
                    }
                },
            },
        };
     ...
     */
    private findOperation(ast: unknown): OperationDefinitionNode | null {
     ...
     */
    validate(query: string, variables?: Record<string, unknown>): boolean {
