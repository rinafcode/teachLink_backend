...
export class LocalizationService {
    ...
    private readonly cacheManager: Cache, private readonly configService: ConfigService, private readonly languageDetection: LanguageDetectionService) { }
    ...
    async invalidateBundles(pairs: Array<{
        namespace: string;
        locale: string;
    }>): Promise<void> {
    ...
    private async loadRawBundleFromDb(namespace: string, locale: string): Promise<Record<string, string>> {
    ...
    async getBundleForApi(namespace: string, locale: string): Promise<{
        locale: string;
        namespace: string;
    ...
    async update(id: string, dto: UpdateTranslationDto): Promise<TranslationListItemDto> {
        ...
        if (dto.namespace !== undefined)
            row.namespace = dto.namespace.trim();
        if (dto.key !== undefined)
            row.translationKey = dto.key.trim();
        if (dto.locale !== undefined)
            row.locale = languageDetectionNormalize(dto.locale);
    ...
    async importRows(rows: TranslationImportRowDto[]): Promise<{
        upserted: number;
    }> {
        if (!rows?.length) {
    ...
    async exportRows(namespace: string, locale?: string): Promise<Array<{
        namespace: string;
        key: string;
        locale: string;
        value: string;
    ...
    static toCsv(rows: Array<{
        ...
        const lines = rows.map((r) => [csvEscape(r.namespace), csvEscape(r.key), csvEscape(r.locale), csvEscape(r.value)].join(','));
