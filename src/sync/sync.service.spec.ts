...
import { SyncEvent, SyncEventType, SyncStatus, DataSource } from "./entities/sync-event.entity"
import { ConflictLog, ConflictType, ResolutionStrategy } from "./entities/conflict-log.entity"
import { IntegrityCheck, CheckType, CheckStatus } from "./entities/integrity-check.entity"
...
describe("DataSyncService", () => {
  ...
  beforeEach(async () => {
    ...
    syncEventRepository = module.get<Repository<SyncEvent>>(getRepositoryToken(SyncEvent))
  ...
  describe("createSyncEvent", () => {
    it("should create and queue a sync event", async () => {
      ...
      jest.spyOn(service as any, "generateVersion").mockResolvedValue(1234567890001)
  ...
  describe("processSyncEvent", () => {
    it("should process sync event successfully", async () => {
      ...
      jest.spyOn(service as any, "syncToDataSource").mockResolvedValue(undefined)
      jest.spyOn(service as any, "updateSyncEventStatus").mockResolvedValue(undefined)
  ...
  describe("bulkSync", () => {
    it("should process multiple entities in batches", async () => {
      ...
      const result = await service.bulkSync("User", entityIds, DataSource.PRIMARY_DB)
...
describe("ConflictResolutionService", () => {
  ...
  beforeEach(async () => {
    ...
    conflictLogRepository = module.get<Repository<ConflictLog>>(getRepositoryToken(ConflictLog))
  })
  ...
  describe("resolveConflict", () => {
    it("should resolve conflict using last write wins strategy", async () => {
      ...
      const result = await service.resolveConflict("User", "user-123", conflictingData, {
        strategy: "last_write_wins",
      })
  ...
  describe("detectConflict", () => {
    it("should detect version conflict", async () => {
      ...
      const conflictType = await service.detectConflict("User", "user-123", newData, existingData)
...
describe("DataConsistencyService", () => {
  ...
  beforeEach(async () => {
    ...
    integrityCheckRepository = module.get<Repository<IntegrityCheck>>(getRepositoryToken(IntegrityCheck))
  })
  ...
  describe("performConsistencyCheck", () => {
    it("should perform consistency check and return results", async () => {
      ...
      const result = await service.performConsistencyCheck("User", ["primary_db", "cache"])
