...
export class ReplicationService {
  ...
  async setupReplication(
    ...
    for (const targetRegion of targetRegions) {
      ...
      this.logger.log(`Replication setup for ${entityType} from ${sourceRegion} to ${targetRegion}`)
    }
  }

  async replicateEvent(syncEvent: SyncEvent, config: ReplicationConfig): Promise<void> {
    if (!config.enabled) return

    const sourceRegion = syncEvent.region
    const targetRegions = config.regions.filter((region) => region !== sourceRegion)

    for (const targetRegion of targetRegions) {
      try {
        await this.replicateToRegion(syncEvent, targetRegion, config)
        await this.updateReplicationStatus(syncEvent.entityType, sourceRegion, targetRegion, {
          lastSyncedVersion: syncEvent.version,
          lastSyncTime: new Date(),
          lagSeconds: 0,
        })
      } catch (error) {
        this.logger.error(`Replication failed from ${sourceRegion} to ${targetRegion}: ${error.message}`)
        await this.updateReplicationStatus(syncEvent.entityType, sourceRegion, targetRegion, {
  ...
  async pauseReplication(entityType: string, sourceRegion: string, targetRegion: string): Promise<void> {
    await this.updateReplicationStatus(entityType, sourceRegion, targetRegion, {
      state: ReplicationState.PAUSED,
    })

    this.logger.log(`Replication paused for ${entityType} from ${sourceRegion} to ${targetRegion}`)
  }

  async resumeReplication(entityType: string, sourceRegion: string, targetRegion: string): Promise<void> {
    await this.updateReplicationStatus(entityType, sourceRegion, targetRegion, {
      state: ReplicationState.ACTIVE,
      lastError: null,
    })
  ...
  async getReplicationLag(entityType: string, sourceRegion: string, targetRegion: string): Promise<number> {
  ...
  async getReplicationStatus(entityType?: string): Promise<ReplicationStatus[]> {
  ...
  async monitorReplicationHealth(): Promise<void> {
    ...
    for (const replication of activeReplications) {
      ...
      if (lag > maxLag) {
        ...
        await this.updateReplicationStatus(replication.entityType, replication.sourceRegion, replication.targetRegion, {
          lagSeconds: lag,
        })
      }
    }
  ...
  private async replicateToRegion(
    ...
    if (!channel) {
      throw new Error(`No replication channel configured for region: ${targetRegion}`)
    }
    ...
    this.logger.debug(`Replicated event ${syncEvent.id} to region ${targetRegion}`)
  }
  ...
  private async calculateReplicationLag(replication: ReplicationStatus): Promise<number> {
