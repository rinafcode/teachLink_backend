...
export class DataConsistencyService {
  ...
  async performConsistencyCheck(
    ...
    const checkId = await this.createIntegrityCheck(
      entityType,
      checkType,
      dataSources,
      startTime,
    ...
    try {
      ...
      switch (checkType) {
        ...
        case CheckType.REFERENTIAL_INTEGRITY:
          result = await this.checkReferentialIntegrity(
            entityType,
            dataSources,
          );
          break;
      ...
      await this.updateIntegrityCheck(checkId, {
        ...
        details: {
          errors: result.inconsistencies.map((e) => ({
            ...e,
  ...
  async checkDataConsistency(
    ...
    for (const entityId of entityIds) {
      ...
      for (const source of dataSources) {
        try {
          const data = await this.getEntityFromSource(
            entityType,
            entityId,
            source,
          );
          ...
          warnings.push(
            `Failed to retrieve ${entityId} from ${source}: ${error.message}`,
          );
        }
      }
  ...
  async checkDataCompleteness(
    ...
    for (const source of dataSources) {
      try {
        ...
        warnings.push(
          `Failed to get entity IDs from ${source}: ${error.message}`,
        );
      }
    }
  ...
  async checkReferentialIntegrity(
    ...
    for (const source of dataSources) {
      ...
      for (const entityId of entityIds) {
        recordsChecked++;
        const entity = await this.getEntityFromSource(
          entityType,
          entityId,
          source,
        );
  ...
  async performScheduledConsistencyChecks(): Promise<void> {
    ...
    for (const entityType of entityTypes) {
      try {
        ...
        this.logger.error(
          `Consistency check failed for ${entityType}: ${error.message}`,
        );
      }
    }
  }
  ...
  private async updateIntegrityCheck(
    checkId: string,
    updates: Partial<IntegrityCheck>,
  ): Promise<void> {
  ...
  private async getEntityFromSource(
    entityType: string,
    entityId: string,
    source: string,
  ): Promise<any> {
  ...
  private async getReferenceFields(
    entityType: string,
  ): Promise<Array<{ field: string; targetEntity: string }>> {
