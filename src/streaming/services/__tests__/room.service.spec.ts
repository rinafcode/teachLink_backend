...
describe('RoomService', () => {
  ...
  describe('startRoom', () => {
    ...
    it('should throw ForbiddenException if not instructor', async () => {
      ...
      await expect(service.startRoom('room-1', 'other-user')).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('should throw BadRequestException if room not scheduled', async () => {
      ...
      await expect(service.startRoom('room-1', 'instructor-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('endRoom', () => {
    it('should end room successfully', async () => {
      ...
      expect(mockParticipantService.endAllParticipants).toHaveBeenCalledWith(
        'room-1',
      );
    });
    ...
    it('should throw ForbiddenException if not instructor', async () => {
      ...
      await expect(service.endRoom('room-1', 'other-user')).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
