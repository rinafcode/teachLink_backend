...
export class MediaStreamingService {
  ...
  async startStream(
    ...
    try {
      ...
      this.logger.error(
        `Failed to start stream for room ${config.roomId}:`,
        error,
      );
      throw error;
    }
  ...
  async stopStream(
    ...
    try {
      ...
      streamStats.duration = Math.floor(
        (streamStats.endTime.getTime() - streamStats.startTime.getTime()) /
          1000,
      );
    ...
    const updateInterval = setInterval(() => {
      ...
      if (stats && stats.isLive) {
        ...
        stats.viewers = Math.max(
          0,
          stats.viewers + Math.floor(Math.random() * 5) - 2,
        );
