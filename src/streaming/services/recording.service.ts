...
export class RecordingService {
  ...
  constructor(recordingRepository: Repository<Recording>, roomRepository: Repository<Room>) {
    this.recordingRepository = recordingRepository
    this.roomRepository = roomRepository
  }

  async startRecording(roomId: string, instructorId: string): Promise<Recording> {
  ...
  async stopRecording(roomId: string, instructorId: string): Promise<Recording> {
    const room = await this.roomRepository.findOne({ where: { id: roomId } })
    if (!room) {
      throw new NotFoundException("Room not found")
    }
    ...
    const duration = Math.floor((recording.endedAt.getTime() - recording.startedAt.getTime()) / 1000)
    ...
    setTimeout(async () => {
      try {
        ...
        if (recording) {
          ...
          recording.fileUrl = `/api/recordings/${recordingId}/download`
  ...
  async deleteRecording(recordingId: string, instructorId: string): Promise<void> {
    const recording = await this.recordingRepository.findOne({
      where: { id: recordingId },
      relations: ["room"],
    })
