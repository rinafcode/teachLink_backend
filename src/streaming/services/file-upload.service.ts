...
export class FileUploadService {
  ...
  private readonly allowedMimeTypes = {
    ...
    [FileCategory.VIDEO]: ["video/mp4", "video/webm", "video/ogg", "video/avi", "video/mov"],
    [FileCategory.AUDIO]: ["audio/mp3", "audio/wav", "audio/ogg", "audio/aac", "audio/m4a"],
  ...
  private validateFile(file: UploadedFile, category: FileCategory): void {
    ...
    if (!allowedTypes.includes(file.mimetype)) {
      throw new BadRequestException(`File type ${file.mimetype} not allowed for category ${category}`)
    }
  ...
  private isExecutableFile(filename: string): boolean {
    const executableExtensions = [".exe", ".bat", ".cmd", ".com", ".scr", ".pif", ".js", ".vbs", ".jar"]
  ...
  async getFile(category: FileCategory, filename: string): Promise<{ buffer: Buffer; mimetype: string }> {
  ...
  async uploadAvatar(file: UploadedFile, userId: string): Promise<FileMetadata> {
  ...
  async uploadDocument(file: UploadedFile, userId: string): Promise<FileMetadata> {
    return this.uploadFile(file, FileCategory.DOCUMENT, userId)
  }

  async uploadRecording(file: UploadedFile, userId: string): Promise<FileMetadata> {
