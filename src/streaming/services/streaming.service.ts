...
export class StreamingService {
  ...
  async updateStreamingStats(
    roomId: string,
    stats: Partial<StreamingStats>,
  ): Promise<void> {
  ...
  async getStreamingStats(roomId: string): Promise<StreamingStats> {
    ...
    if (room.actualStartTime) {
      stats.duration = Math.floor(
        (Date.now() - room.actualStartTime.getTime()) / 1000,
      );
    }
  ...
  async endStream(roomId: string, instructorId: string): Promise<void> {
    ...
    ).filter(([connectionId]) => connectionId.startsWith(roomId));
  ...
  async getStreamingHealth(roomId: string): Promise<{
    ...
    try {
      ...
      const activeConnections = Array.from(
        this.peerConnections.entries(),
      ).filter(([connectionId]) => connectionId.startsWith(roomId)).length;
