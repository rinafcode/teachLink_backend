...
import { API_VERSION_HEADER, DEFAULT_API_VERSION, SUPPORTED_API_VERSIONS, } from './common/interceptors/api-version.interceptor';
...
async function bootstrapWorker(): Promise<void> {
    ...
    app.use((req: unknown, res: unknown, next: unknown) => {
        ...
        if (!req.session.userAgent) {
            ...
            return req.session.destroy((err: unknown) => {
                if (err) {
    ...
    if (sessionConfig.stickySessionsRequired) {
        logger.log('Sticky sessions are enabled by policy. Configure LB cookie affinity on teachlink.sid.');
    }
    logger.log(`🚀 TeachLink API running on http://localhost:${port}`);
    logger.log(`📚 Swagger docs available at http://localhost:${port}/api`);
    logger.log(`🧭 API versioning enabled via ${API_VERSION_HEADER}. Supported versions: ${SUPPORTED_API_VERSIONS.join(', ')}; default route version: ${DEFAULT_API_VERSION}.`);
...
async function bootstrap(): Promise<void> {
    ...
    if (clusterModeEnabled && cluster.isPrimary) {
        ...
        cluster.on('exit', (worker, code, signal) => {
            if (isShuttingDown) {
                logger.log(`Worker ${worker.id} (${worker.process.pid}) exited during shutdown (code: ${code}, signal: ${signal}).`);
            ...
            logger.warn(`Worker ${worker.id} (${worker.process.pid}) died (code: ${code}, signal: ${signal}). Restarting...`);
            cluster.fork();
        });
        const shutdownCluster = (signal: string): void => {
            ...
            isShuttingDown = true;
            logger.log(`Primary received ${signal}. Shutting down ${Object.keys(cluster.workers || {}).length} workers...`);
