import { Controller, Post, Get, Delete, Param, Query, Body, UploadedFile, UseInterceptors, HttpException, HttpStatus, Logger, BadRequestException, } from '@nestjs/common';
...
import { FileValidationService, FileValidationResult, } from '../media/validation/file-validation.service';
...
export class CdnController {
    ...
    constructor(private readonly cdnService: CdnService, private readonly fileValidation: FileValidationService, private readonly malwareScanning: MalwareScanningService, private readonly imageProcessing: ImageProcessingService) { }
    @Post('upload')
    ...
    file: FileUpload, 
    @Body()
    options: UploadContentDto): Promise<ContentMetadata> {
        try {
            if (!file) {
            ...
            throw new HttpException(`Upload failed: ${error instanceof Error ? error.message : 'Unknown error'}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    @Post('validate')
    @UseInterceptors(FileInterceptor('file'))
    ...
    async validateFile(
    @UploadedFile()
    file: FileUpload): Promise<FileValidationResult> {
        if (!file) {
