...
export class EdgeCachingService {
  ...
  async purgeAsset(assetId: string, regions?: string[]): Promise<void> {
    const whereCondition: any = {
      key: regions
        ? regions.map((region) => this.generateCacheKey(assetId, region, '%'))
        : `%${assetId}%`,
    };
    ...
    await this.cacheRepository.update(whereCondition, {
      status: CacheStatus.PURGED,
    });
  ...
  async warmCache(
    ...
    const promises = regions.map((region) =>
      this.cacheAsset(assetId, url, { region }),
    );

    await Promise.all(promises);
    this.logger.log(
      `Cache warmed for asset ${assetId} in ${regions.length} regions`,
    );
  }
