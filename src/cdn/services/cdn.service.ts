...
import type {
  CDNResponse,
  OptimizationOptions,
} from '../interfaces/cdn.interfaces';
...
export class CDNService {
  ...
  async uploadAsset(
    ...
    try {
      ...
      if (type === AssetType.IMAGE && options) {
        optimizedBuffer = await this.assetOptimizationService.optimizeImage(
          file,
          options,
        );
      ...
      const cdnUrls = await this.uploadToProviders(
        optimizedBuffer,
        filename,
        asset.id,
      );
  ...
  async getOptimizedUrl(
    ...
    const cachedUrl = await this.edgeCachingService.getCachedUrl(
      asset.id,
      region,
      provider.type,
    );
    ...
    if (connectionType && asset.type === AssetType.IMAGE) {
      finalUrl = await this.applyBandwidthOptimization(
        finalUrl,
        connectionType,
      );
    }
  ...
  private async uploadToProviders(
    buffer: Buffer,
    filename: string,
    assetId: string,
  ): Promise<string[]> {
    ...
    for (const provider of providers) {
      try {
        ...
        this.logger.warn(
          `Failed to upload to provider ${provider.name}: ${error.message}`,
        );
      }
    }
  ...
  private async getOptimalProvider(region: string): Promise<CDNProvider> {
    ...
    const regionalProvider = providers.find(
