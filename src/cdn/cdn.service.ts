...
export class CdnService {
    ...
    private contentMetadataRepository: Repository<ContentMetadata>, private assetOptimizationService: AssetOptimizationService, private edgeCachingService: EdgeCachingService, private geoLocationService: GeoLocationService, private cloudflareService: CloudflareService) { }
    ...
    async uploadContent(file: UploadedFile, options: ContentDeliveryOptions = {}): Promise<ContentMetadata> {
        try {
    ...
    private async optimizeContentAsync(metadata: ContentMetadata, options: ContentDeliveryOptions): Promise<void> {
        try {
            metadata.status = ContentStatus.PROCESSING;
            await this.contentMetadataRepository.save(metadata);
            const _optimizedUrl = await this.assetOptimizationService.optimizeImage(metadata.cdnUrl, options);
