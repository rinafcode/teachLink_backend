...
export class GeoLocationService {
    ...
    async getOptimalLocation(userLocation?: string): Promise<string> {
        ...
        let minDistance = this.calculateDistance(userCoords.latitude, userCoords.longitude, optimalLocation.latitude, optimalLocation.longitude);
        for (const location of this.edgeLocations.slice(1)) {
            const distance = this.calculateDistance(userCoords.latitude, userCoords.longitude, location.latitude, location.longitude);
    ...
    private async getCoordinates(location: string): Promise<{
        latitude: number;
        longitude: number;
    } | null> {
        // In real implementation, use geocoding service
        // For now, return mock coordinates
        const mockCoords: Record<string, {
            latitude: number;
            longitude: number;
        }> = {
