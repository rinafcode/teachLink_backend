...
export class CloudflareService {
    ...
    async getAnalytics(startDate: Date, endDate: Date): Promise<unknown> {
        try {
            const response = await this.httpClient.get(`/zones/${this.config.zoneId}/analytics/dashboard`, {
                params: {
    ...
    async createCustomDomain(domain: string): Promise<boolean> {
        try {
            const response = await this.httpClient.post(`/zones/${this.config.zoneId}/custom_certificates`, {
    ...
    async getZoneSettings(): Promise<unknown> {
        try {
    ...
    async updateCacheSettings(settings: unknown): Promise<boolean> {
        try {
            const response = await this.httpClient.patch(`/zones/${this.config.zoneId}/settings/cache_level`, {
    ...
    async getImageVariants(imageId: string): Promise<string[]> {
        try {
            const response = await this.httpClient.get(`/accounts/${this.config.accountId}/images/v1/${imageId}/variants`);
    ...
    async deleteImage(imageId: string): Promise<boolean> {
        try {
            const response = await this.httpClient.delete(`/accounts/${this.config.accountId}/images/v1/${imageId}`);
            return response.data.success;
        }
        catch (error) {
