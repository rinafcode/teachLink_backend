...
export class CDNController {
  ...
  async getAsset(
    ...
    @Query('connection') connectionType?: string,
    request?: Request,
  ) {
  ...
  async getNearestEdge(@Query('ip') ip: string) {
    ...
    const nearestEdge =
      await this.geoLocationService.getNearestEdgeLocation(location);
  ...
  async generateResponsiveImages(
    file: Express.Multer.File,
    @Query('breakpoints') breakpoints?: string,
  ) {
    if (!file) {
      throw new BadRequestException('No file provided');
    }
    ...
    const breakpointList = breakpoints
      ? breakpoints.split(',').map((b) => Number.parseInt(b))
      : undefined;

    const results =
      await this.assetOptimizationService.generateResponsiveImages(
        file.buffer,
        breakpointList,
      );
