...
export class DistributedTracingService {
     ...
     */
    async executeInSpan<T>(name: string, fn: (span: Span) => Promise<T>, attributes?: Record<string, unknown>): Promise<T> {
     ...
     * Trace HTTP request
     */
    async traceHttpRequest<T>(method: string, url: string, fn: (span: Span) => Promise<T>): Promise<T> {
     ...
     */
    async traceExternalCall<T>(serviceName: string, operation: string, fn: (span: Span) => Promise<T>): Promise<T> {
