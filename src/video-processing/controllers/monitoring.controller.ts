...
export class MonitoringController {
  ...
  @ApiResponse({ status: 200, description: "Health status retrieved", type: HealthCheckDto })
  ...
  @ApiResponse({ status: 200, description: "Job type metrics retrieved", type: [JobTypeMetricsDto] })
  ...
  @ApiResponse({ status: 200, description: "Processing trends retrieved", type: [ProcessingTrendDto] })
  async getProcessingTrends(query: MonitoringQueryDto): Promise<ProcessingTrendDto[]> {
    return await this.monitoringService.getProcessingTrends(query.days)
  }

  @Get("errors")
  @ApiOperation({ summary: "Get error analysis" })
  @ApiResponse({ status: 200, description: "Error analysis retrieved", type: [ErrorAnalysisDto] })
  ...
  @ApiResponse({ status: 200, description: "Queue statistics retrieved", type: [QueueHealthDto] })
  async getQueueStats() {
  ...
  async getJobs(query: JobQueryDto) {
    const { page, limit, status, type, videoId, createdAfter, createdBefore } = query

    const queryBuilder = this.jobRepository.createQueryBuilder("job").leftJoinAndSelect("job.video", "video")
    ...
    if (createdBefore) {
      queryBuilder.andWhere("job.createdAt <= :createdBefore", { createdBefore })
    }
