...
export class VideoController {
  ...
  @ApiResponse({ status: 201, description: "Video uploaded successfully", type: UploadResponseDto })
  ...
  async uploadVideo(file: Express.Multer.File, @Body() uploadVideoDto: UploadVideoDto): Promise<UploadResponseDto> {
    if (!file) {
  ...
  @ApiResponse({ status: 200, description: "Processing started", type: ProcessingResultResponseDto })
  ...
  @ApiResponse({ status: 200, description: "Videos retrieved successfully", type: [VideoResponseDto] })
  async getVideos(@Query() query: VideoQueryDto) {
    const { page, limit, status, type, courseId, uploadedBy, search, createdAfter, createdBefore, sortBy, sortOrder } =
      query
    ...
    if (createdAfter) {
      queryBuilder.andWhere("video.createdAt >= :createdAfter", { createdAfter })
    }

    if (createdBefore) {
      queryBuilder.andWhere("video.createdAt <= :createdBefore", { createdBefore })
    }
  ...
  @ApiResponse({ status: 404, description: "Video not found" })
  async getVideo(@Param("id", ParseUUIDPipe) id: string): Promise<VideoResponseDto> {
  ...
  @ApiParam({ name: "id", description: "Video ID" })
  @ApiResponse({ status: 200, description: "Status retrieved successfully", type: ProcessingStatusResponseDto })
  @ApiResponse({ status: 404, description: "Video not found" })
  async getProcessingStatus(@Param("id", ParseUUIDPipe) id: string): Promise<ProcessingStatusResponseDto> {
    ...
    try {
      ...
      for (const variant of video.variants || []) {
        try {
          ...
          this.logger.warn(`Failed to delete variant file: ${variant.filePath}`)
        }
      }
  ...
  @ApiParam({ name: "id", description: "Video ID" })
  @ApiResponse({ status: 200, description: "Processing cancelled successfully" })
  @ApiResponse({ status: 404, description: "Video not found" })
  ...
  @ApiParam({ name: "id", description: "Video ID" })
  @ApiResponse({ status: 200, description: "Processing retry started", type: ProcessingResultResponseDto })
  @ApiResponse({ status: 404, description: "Video not found" })
