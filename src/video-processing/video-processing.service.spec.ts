...
import { VideoVariant, VideoQuality, VideoFormat, VariantStatus } from "./entities/video-variant.entity"
import { VideoProcessingJob, JobType, JobStatus, JobPriority } from "./entities/video-processing-job.entity"
...
describe("VideoProcessingService", () => {
  ...
  describe("processVideo", () => {
    it("should successfully process a video", async () => {
      ...
      expect(videoRepository.update).toHaveBeenCalledWith("test-video-id", { status: VideoStatus.PROCESSING })
      expect(metadataService.extractMetadata).toHaveBeenCalledWith(mockVideo.originalFilePath)
    })
    ...
    it("should throw BadRequestException if video not found", async () => {
      ...
      // Act & Assert
      await expect(service.processVideo("non-existent-id")).rejects.toThrow(BadRequestException)
    })
    ...
    it("should throw BadRequestException if video is already processing", async () => {
      ...
      // Act & Assert
      await expect(service.processVideo("test-video-id")).rejects.toThrow(BadRequestException)
    })
    ...
    it("should handle processing errors gracefully", async () => {
      ...
      videoRepository.findOne.mockResolvedValue(mockVideo)
      videoRepository.update.mockResolvedValue({ affected: 1 } as any)
      metadataService.extractMetadata.mockRejectedValue(new Error("Metadata extraction failed"))

      // Act & Assert
      await expect(service.processVideo("test-video-id")).rejects.toThrow(InternalServerErrorException)
  ...
  describe("getProcessingStatus", () => {
    ...
    it("should throw BadRequestException if video not found", async () => {
      ...
      // Act & Assert
      await expect(service.getProcessingStatus("non-existent-id")).rejects.toThrow(BadRequestException)
    })
  })
  ...
  describe("cancelProcessing", () => {
    it("should cancel video processing", async () => {
      ...
      // Assert
      expect(videoRepository.update).toHaveBeenCalledWith("test-video-id", { status: VideoStatus.FAILED })
...
describe("VideoController", () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get(require("./controllers/video.controller").VideoController)
    videoRepository = module.get(getRepositoryToken(Video))
  ...
  describe("uploadVideo", () => {
    ...
    it("should throw BadRequestException if no file provided", async () => {
      ...
      // Act & Assert
      await expect(controller.uploadVideo(null, uploadDto)).rejects.toThrow(BadRequestException)
    })
  })
  ...
  describe("processVideo", () => {
    it("should start video processing", async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.videoId).toBe("test-video-id")
      expect(videoProcessingService.processVideo).toHaveBeenCalledWith("test-video-id", processDto)
    })
  })
  ...
  describe("getProcessingStatus", () => {
    it("should return processing status", async () => {
      ...
      expect(videoProcessingService.getProcessingStatus).toHaveBeenCalledWith("test-video-id")
    })
  })
})
  ...
  describe("ValidationUtils", () => {
    ...
    describe("validateUUID", () => {
      it("should validate UUIDs correctly", () => {
        expect(ValidationUtils.validateUUID("123e4567-e89b-12d3-a456-426614174000")).toBe(true)
    ...
    describe("sanitizeFilename", () => {
      it("should sanitize filenames correctly", () => {
        expect(ValidationUtils.sanitizeFilename("My Video File.mp4")).toBe("my_video_file.mp4")
        expect(ValidationUtils.sanitizeFilename('Video<>:"/\\|?*.mp4')).toBe("video_.mp4")
        expect(ValidationUtils.sanitizeFilename("  Multiple   Spaces  .mp4")).toBe("multiple_spaces.mp4")
      })
    })
  })
