...
export class VideoProcessingService {
  ...
  async processVideo(
    ...
    if (video.status === VideoStatus.PROCESSING) {
      throw new BadRequestException(
        `Video ${videoId} is already being processed`,
      );
    }
    ...
    try {
      ...
      this.logger.error(
        `Video processing failed for video: ${videoId}`,
        error.stack,
      );
    ...
    for (const quality of qualities) {
      for (const format of formats) {
        try {
          ...
          this.logger.error(
            `Failed to process variant ${quality}/${format}`,
            error.stack,
          );
  ...
  private async processVariant(
    ...
    this.logger.log(
      `Processing variant: ${quality}/${format} for video ${video.id}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Variant processing completed: ${quality}/${format} for video ${video.id}`,
      );
      return variant;
    } catch (error) {
  ...
  private async generateAdaptiveStreaming(
    video: Video,
    variants: VideoVariant[],
  ): Promise<void> {
    const mp4Variants = variants.filter(
      (v) =>
        v.format === VideoFormat.MP4 && v.status === VariantStatus.COMPLETED,
    );
    ...
    const hlsPath = await this.generateVariantPath(
      video.id,
      VideoQuality.HIGH,
      VideoFormat.HLS,
    );
    ...
    const dashPath = await this.generateVariantPath(
      video.id,
      VideoQuality.HIGH,
      VideoFormat.DASH,
    );
  ...
  private async updateVariantProgress(
    variantId: string,
    progress: number,
  ): Promise<void> {
  ...
  private async generateVariantPath(
    videoId: string,
    quality: VideoQuality,
    format: VideoFormat,
  ): Promise<string> {
  ...
  async getProcessingStatus(videoId: string) {
    ...
    const completedJobs = video.processingJobs.filter(
      (job) => job.status === JobStatus.COMPLETED,
    ).length;
    const failedJobs = video.processingJobs.filter(
    ...
    return {
      ...
      variants: video.variants.map((variant) => ({
        ...
        processingProgress: variant.processingProgress,
        ...
        codec: variant.codec,
        processingCompletedAt: variant.processingCompletedAt,
      })),
  ...
  async cancelProcessing(videoId: string): Promise<void> {
    ...
    await this.jobRepository.update(
      { videoId, status: JobStatus.QUEUED },
