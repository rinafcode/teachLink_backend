...
export class WorkerService {
  ...
  private async processTranscode(job: VideoProcessingJob): Promise<any> {
    ...
    if (!quality || !format) {
      throw new Error(
        'Quality and format parameters are required for transcode job',
      );
    }
    ...
    const result = await this.ffmpegService.transcode(
      ...
      async (progress) => {
        ...
        await this.updateJobProgress(
          job.id,
          Math.min(5 + Math.floor(progress * 0.9), 95),
        );
      },
    );
  ...
  private async processAdaptiveStreaming(
    job: VideoProcessingJob,
  ): Promise<any> {
    const { video } = job;
    if (!video) {
    ...
    const variants =
      video.variants?.filter(
        (v) => v.format === 'mp4' && v.status === 'completed',
      ) || [];
  ...
  private generateQualityRecommendations(metadata: any): string[] {
    ...
    if (pixelCount < 1280 * 720) {
      recommendations.push(
        'Consider uploading higher resolution content (720p or above)',
      );
    }
    ...
    if (metadata.frameRate < 30) {
      recommendations.push(
        'Consider using 30fps or higher for smoother playback',
      );
    }
