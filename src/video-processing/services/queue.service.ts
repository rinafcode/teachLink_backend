...
import { type VideoProcessingJob, JobStatus, JobPriority } from "../entities/video-processing-job.entity"
import { type ProcessingQueue, QueueStatus } from "../entities/processing-queue.entity"
...
export class QueueService implements OnModuleInit, OnModuleDestroy {
  ...
  private async processQueue(queue: ProcessingQueue) {
    ...
    const jobsForQueue = queuedJobs.filter((job) => this.getQueueNameForJob(job) === queue.name)
  ...
  private async startJobProcessing(job: VideoProcessingJob, queue: ProcessingQueue) {
  ...
  private async handleJobError(job: VideoProcessingJob, error: Error) {
    ...
    if (job.retryCount < job.maxRetries) {
      ...
      this.logger.warn(`Job ${job.id} failed, scheduling retry ${job.retryCount}/${job.maxRetries}`)
    } else {
      // Mark as failed
  ...
  private getQueueNameForJob(job: VideoProcessingJob): string {
    switch (job.priority) {
      ...
      case JobPriority.NORMAL:
        return job.type === "thumbnail_generation" ? "thumbnail-generation" : "normal-priority"
  ...
  private async calculateQueueStats(queue: ProcessingQueue): Promise<QueueStats> {
    const queueName = this.getQueueNameForJob({ priority: queue.priority } as VideoProcessingJob)
    ...
    const averageProcessingTime =
      completedJobsWithDuration.length > 0
        ? completedJobsWithDuration.reduce((sum, job) => sum + (job.actualDuration || 0), 0) /
  ...
  private async gracefulShutdown() {
    ...
    if (this.activeJobs.size > 0) {
      this.logger.warn(`Forcing shutdown with ${this.activeJobs.size} active jobs remaining`)
