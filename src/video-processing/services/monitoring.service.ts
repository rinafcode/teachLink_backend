...
export class MonitoringService {
  ...
  async getProcessingTrends(days = 7): Promise<any> {
    ...
    const dailyStats = await this.jobRepository
      ...
      .addSelect(
        "SUM(CASE WHEN job.status = 'failed' THEN 1 ELSE 0 END)",
        'failedJobs',
      )
    ...
    return dailyStats.map((stat) => ({
      ...
      successRate:
        stat.totalJobs > 0 ? (stat.completedJobs / stat.totalJobs) * 100 : 0,
    }));
  }
  ...
  private async calculateAverageProcessingTimeByType(
    type: JobType,
  ): Promise<number> {
    const result = await this.jobRepository
      .createQueryBuilder('job')
  ...
  private async getQueueHealth(): Promise<QueueHealth[]> {
    ...
    for (const queue of queues) {
      const utilizationPercent =
        queue.maxConcurrentJobs > 0
          ? (queue.currentActiveJobs / queue.maxConcurrentJobs) * 100
          : 0;
  ...
  private categorizeError(error: string): string {
    ...
    if (error.includes('file') || error.includes('path'))
      return 'File System Error';
    if (error.includes('memory') || error.includes('space'))
  ...
  async getHealthCheck(): Promise<{ status: string; details: any }> {
    try {
      ...
      const isHealthy =
        metrics.systemLoad < 90 && metrics.failedJobs < metrics.totalJobs * 0.1;
