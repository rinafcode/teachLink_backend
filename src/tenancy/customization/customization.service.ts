...
export class CustomizationService {
     ...
     */
    async updateCustomization(tenantId: string, updateDto: UpdateTenantCustomizationDto): Promise<TenantCustomization> {
     ...
     * Update theme colors
     */
    async updateColors(tenantId: string, colors: {
        primary?: string;
        secondary?: string;
        accent?: string;
        ...
        const customization = await this.getCustomization(tenantId);
        if (colors.primary)
            customization.primaryColor = colors.primary;
        if (colors.secondary)
            customization.secondaryColor = colors.secondary;
        if (colors.accent)
            customization.accentColor = colors.accent;
        return await this.customizationRepository.save(customization);
    }
    /**
     * Update theme configuration
     */
    async updateTheme(tenantId: string, theme: Record<string, unknown>): Promise<TenantCustomization> {
        const customization = await this.getCustomization(tenantId);
        customization.theme = {
            ...customization.theme,
            ...theme,
     ...
     */
    async updateEmailTemplates(tenantId: string, templates: Record<string, string>): Promise<TenantCustomization> {
        const customization = await this.getCustomization(tenantId);
     ...
     * Update social links
     */
    async updateSocialLinks(tenantId: string, socialLinks: Record<string, string>): Promise<TenantCustomization> {
        const customization = await this.getCustomization(tenantId);
     ...
     */
    async updateLandingPage(tenantId: string, config: Record<string, unknown>): Promise<TenantCustomization> {
        const customization = await this.getCustomization(tenantId);
