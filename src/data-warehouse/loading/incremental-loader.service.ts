...
export class IncrementalLoaderService {
  ...
  async executeLoad(
    ...
    private async loadByTimestamp(job: IncrementalLoadJob): Promise<{
        ...
        const incrementalData = await this.getSourceDataSince(job.config.sourceConnection, job.sourceTable, timestampColumn, lastTimestamp);
        // Apply changes to target
        const result = await this.applyChangesToTarget(job.config.targetConnection, job.targetTable, incrementalData, job.config.primaryKey, 'timestamp');
    ...
    private async loadBySequence(job: IncrementalLoadJob): Promise<{
        ...
        // Get incremental data from source
        const incrementalData = await this.getSourceDataAfter(job.config.sourceConnection, job.sourceTable, sequenceColumn, lastId);
        // Apply changes to target
        const result = await this.applyChangesToTarget(job.config.targetConnection, job.targetTable, incrementalData, job.config.primaryKey, 'sequence');
    ...
    private async loadByWatermark(job: IncrementalLoadJob): Promise<{
        ...
        // Get incremental data from source
        const incrementalData = await this.getSourceDataAfter(job.config.sourceConnection, job.sourceTable, watermarkColumn, lastValue);
        // Apply changes to target
        const result = await this.applyChangesToTarget(job.config.targetConnection, job.targetTable, incrementalData, job.config.primaryKey, 'watermark');
        // Update watermark
        if (incrementalData.length > 0) {
  ...
  private async loadByWatermark(job: IncrementalLoadJob): Promise<{
     ...
     */
    async setWatermark(tableName: string, columnName: string, value: unknown): Promise<Watermark> {
        const key = `${tableName}_${columnName}`;
    ...
    private async getSourceDataSince(_connection: DataSourceConfig, table: string, column: string, timestamp: Date): Promise<unknown[]> {
    ...
    private async getSourceDataAfter(_connection: DataSourceConfig, table: string, column: string, value: unknown): Promise<unknown[]> {
        // Implementation would connect to source database and query
        this.logger.log(`Querying ${table} where ${column} > ${value}`);
        return []; // Placeholder
    }
    private async applyChangesToTarget(_connection: DataSourceConfig, table: string, data: unknown[], _primaryKey: string[], loadType: string): Promise<{
        processed: number;
        inserted: number;
        updated: number;
    }> {
    ...
    private async insertRecord(_connection: DataSourceConfig, table: string, _values: {
        [key: string]: unknown;
    }): Promise<void> {
        // Implementation would insert record into target
        this.logger.log(`Inserting record into ${table}`);
    }
    private async validateRecord(_values: unknown): Promise<boolean> {
        // Implementation would validate record
        return true;
    }
    private async updateRecord(_connection: DataSourceConfig, table: string, primaryKey: {
        [key: string]: unknown;
    }, _values: {
        [key: string]: unknown;
    }): Promise<void> {
    ...
    private async deleteRecord(_connection: DataSourceConfig, table: string, primaryKey: {
        [key: string]: unknown;
    }): Promise<void> {
