...
export class DataQualityService {
     ...
     */
    async runQualityChecks(profileId: string, data: unknown[]): Promise<DataQualityCheck> {
        const profile = this.profiles.get(profileId);
        if (!profile) {
     ...
     */
    private async createQualityIssue(profileId: string, rule: DataQualityRule, result: DataQualityResult): Promise<DataQualityIssue> {
     ...
     * Get quality issues
     */
    async getQualityIssues(profileId?: string, severity?: string, resolved?: boolean): Promise<DataQualityIssue[]> {
    ...
    private calculateCompleteness(data: unknown[], field: string): number {
        if (data.length === 0)
            return 100;
    ...
    private calculateUniqueness(data: unknown[], field: string): number {
        if (data.length === 0)
            return 100;
        const uniqueValues = new Set(data.map((item) => item[field]));
        return (uniqueValues.size / data.length) * 100;
    }
    private calculateValidity(data: unknown[], field: string, condition: string): number {
        if (data.length === 0)
            return 100;
        const validCount = data.filter((item) => this.isValid(item[field], condition)).length;
        return (validCount / data.length) * 100;
    }
    private isValid(value: unknown, condition: string): boolean {
    ...
    private findDuplicates(data: unknown[], field: string): unknown[] {
        const seen = new Map();
