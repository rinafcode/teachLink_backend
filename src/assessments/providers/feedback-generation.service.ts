...
export class FeedbackGenerationService {
  ...
  private async generateQuestionFeedback(
    ...
    return results.map((result) => ({
      ...
      feedback:
        result.feedback || this.generateQuestionSpecificFeedback(result),
  ...
  private generateMultipleChoiceFeedback(
    question: Question,
    userAnswer: string,
  ): string {
  ...
  private generateTrueFalseFeedback(
    question: Question,
    userAnswer: boolean,
  ): string {
    const correctOption = question.options.find((opt) => opt.isCorrect);
  ...
  private generateShortAnswerFeedback(
    question: Question,
    userAnswer: string,
  ): string {
  ...
  private generateCodingFeedback(
    question: Question,
    userAnswer: string,
  ): string {
  ...
  private identifyStrengths(results: AssessmentResult[]): string[] {
    ...
    const correctByType = this.groupResultsByType(
      results.filter((r) => r.isCorrect),
    );
    const correctByDifficulty = this.groupResultsByDifficulty(
      results.filter((r) => r.isCorrect),
    );

    // Identify strong question types
    Object.entries(correctByType).forEach(([type, count]) => {
      const totalOfType = results.filter(
        (r) => r.question.type === type,
      ).length;
      if (count / totalOfType >= 0.8) {
        strengths.push(
          `Strong performance in ${type.replace('_', ' ')} questions`,
        );
      }
    });
    ...
    Object.entries(correctByDifficulty).forEach(([difficulty, count]) => {
      const totalOfDifficulty = results.filter(
        (r) => r.question.difficulty === difficulty,
      ).length;
  ...
  private identifyWeaknesses(results: AssessmentResult[]): string[] {
    ...
    const incorrectByType = this.groupResultsByType(
      results.filter((r) => !r.isCorrect),
    );
    const incorrectByDifficulty = this.groupResultsByDifficulty(
      results.filter((r) => !r.isCorrect),
    );
    ...
    Object.entries(incorrectByType).forEach(([type, count]) => {
      ...
      if (count / totalOfType >= 0.5) {
        weaknesses.push(
          `Need improvement in ${type.replace('_', ' ')} questions`,
        );
      }
    });
  ...
  private generateRecommendations(
    ...
    if (attempt.percentage < 60) {
      recommendations.push(
        'Review the fundamental concepts before retaking the assessment',
      );
      recommendations.push(
        'Consider seeking additional help from your instructor',
      );
    ...
    if (attempt.timeSpent && attempt.assessment.timeLimit) {
      ...
      if (timeRatio > 0.9) {
        ...
        recommendations.push(
          'Take more time to carefully read and analyze each question',
        );
      }
    }

    // Type-specific recommendations
    const codingErrors = incorrectResults.filter(
      (r) => r.question.type === QuestionType.CODING_CHALLENGE,
    );
    if (codingErrors.length > 0) {
      recommendations.push(
        'Practice more coding exercises and review programming fundamentals',
      );
    }
  ...
  private groupResultsByDifficulty(
    results: AssessmentResult[],
  ): Record<string, number> {
    return results.reduce(
      (acc, result) => {
