...
export class ScoreCalculationService {
  async calculateQuestionScore(question: Question, userAnswer: any, timeSpent?: number): Promise<ScoringResult> {
  ...
  private scoreMultipleChoice(question: Question, userAnswer: string, pointsPossible: number): ScoringResult {
    ...
    return {
      ...
      pointsPossible,
      feedback: isCorrect ? "Correct!" : `Incorrect. The correct answer is: ${correctOption?.optionText}`,
    }
  }

  private scoreTrueFalse(question: Question, userAnswer: boolean, pointsPossible: number): ScoringResult {
    const correctOption = question.options.find((option) => option.isCorrect)
  ...
  private scoreShortAnswer(question: Question, userAnswer: string, pointsPossible: number): ScoringResult {
    ...
    const isCorrect = correctAnswers.some((correct: string) => userAnswerLower.includes(correct.toLowerCase()))

    return {
      isCorrect,
      pointsEarned: isCorrect ? pointsPossible : 0,
      pointsPossible,
      feedback: isCorrect ? "Correct!" : "Your answer needs review. Please check with your instructor.",
    }
  }
  ...
  private async scoreCodingChallenge(
    ...
    try {
      ...
      const partialCredit = testCases.length > 0 ? passedTests / testCases.length : 0
  ...
  async calculateAssessmentScore(results: AssessmentResult[]): Promise<{
    ...
    const totalPossible = results.reduce((sum, result) => sum + result.pointsPossible, 0)
  ...
  async calculateTimeBonus(timeSpent: number, timeLimit: number): Promise<number> {
