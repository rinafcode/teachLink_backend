...
import {
  AssessmentAttempt,
  AttemptStatus,
} from '../entities/assessment-attempt.entity';
...
export class AssessmentsService {
  ...
  async createAssessment(
    ...
    const assessment = this.assessmentRepository.create({
      ...createAssessmentDto,
      createdBy,
      availableFrom: createAssessmentDto.availableFrom
        ? new Date(createAssessmentDto.availableFrom)
        : null,
      availableUntil: createAssessmentDto.availableUntil
        ? new Date(createAssessmentDto.availableUntil)
        : null,
    });
    ...
    if (
      ...
      createAssessmentDto.questions.length > 0
    ) {
      for (const questionDto of createAssessmentDto.questions) {
        await this.questionBankService.createQuestion(
          savedAssessment.id,
          questionDto,
        );
      }
  ...
  async findAllAssessments(courseId?: string): Promise<Assessment[]> {
    const where = courseId
      ? { courseId, status: AssessmentStatus.PUBLISHED }
  ...
  async startAssessment(
    ...
    const assessment = await this.findAssessmentById(
      startAssessmentDto.assessmentId,
    );
  ...
  async submitAssessment(
    ...
    const results = await this.processAnswers(
      attempt,
    ...
    const scoreData =
      await this.scoreCalculationService.calculateAssessmentScore(results);
  ...
  async getAssessmentResults(attemptId: string, userId: string): Promise<any> {
    ...
    const feedback =
      await this.feedbackGenerationService.generateComprehensiveFeedback(
        attempt,
        attempt.results,
      );
  ...
  private async validateAssessmentAvailability(
  ...
  private async processAnswers(
    ...
    for (const question of attempt.assessment.questions) {
      ...
      const scoringResult =
        await this.scoreCalculationService.calculateQuestionScore(
          question,
          userAnswer,
        );
