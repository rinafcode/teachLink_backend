...
import { CACHEABLE_KEY, CACHE_KEY_GENERATOR, CACHE_CONDITION } from "../decorators/cacheable.decorator"
...
export class CacheInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
    const cacheOptions = this.reflector.get<CacheOptions>(CACHEABLE_KEY, context.getHandler())
  ...
  private generateCacheKey(context: ExecutionContext, options: CacheOptions): string {
    const keyGenerator = this.reflector.get(CACHE_KEY_GENERATOR, context.getHandler())
  ...
  private hashArgs(args: any[]): string {
    try {
      return Buffer.from(JSON.stringify(args)).toString("base64").substring(0, 16)
    } catch {
      return "default"
    }
  }
