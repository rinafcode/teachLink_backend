...
export class CacheAnalyticsService implements OnModuleInit {
  ...
  async generateHourlyReport(): Promise<void> {
    ...
    this.logger.log(
      `Hourly cache analytics - Hit Ratio: ${analytics.hitRatio.toFixed(2)}%`,
    );

    // Log performance insights
    if (analytics.hitRatio < 80) {
      this.logger.warn(
        `Cache hit ratio below target: ${analytics.hitRatio.toFixed(2)}%`,
      );
    }

    if (analytics.avgResponseTime > 100) {
      this.logger.warn(
        `High average response time: ${analytics.avgResponseTime.toFixed(2)}ms`,
      );
    }
  }
  ...
  async getAnalytics(): Promise<CacheAnalytics> {
    ...
    const memoryUsage = Array.from(this.keyAnalytics.values()).reduce(
      (sum, ka) => sum + ka.size,
      0,
    );
  ...
  private getStrategyPerformance(): StrategyAnalytics[] {
    return Array.from(this.strategyAnalytics.values()).sort(
      (a, b) => b.usage - a.usage,
    );
  }
  ...
  private generateRecommendations(
    ...
    if (hitRatio < 80) {
      ...
      recommendations.push(
        'Review cache TTL settings - they might be too short',
      );
      recommendations.push(
    ...
    if (avgResponseTime > 100) {
      recommendations.push(
        'Consider using local caching for frequently accessed small objects',
      );
    ...
    if (hitRatio > 95) {
      recommendations.push(
        'Excellent cache performance! Consider increasing cache size for more data',
      );
    }

    // Strategy-specific recommendations
    const topStrategy = Array.from(this.strategyAnalytics.values()).sort(
      (a, b) => b.usage - a.usage,
    )[0];
    if (topStrategy && topStrategy.hitRatio < 70) {
      recommendations.push(
        `Consider optimizing ${topStrategy.strategy} strategy performance`,
      );
    }
