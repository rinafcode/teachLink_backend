...
export class CacheInvalidationService implements OnModuleInit, OnModuleDestroy {
    ...
    constructor(private readonly cachingService: CachingService, private readonly strategiesService: CacheStrategiesService, private readonly eventEmitter: EventEmitter2) { }
    onModuleInit() {
     ...
     */
    async invalidateEnrollment(enrollmentId: string, courseId?: string): Promise<InvalidationResult[]> {
    ...
    async handleCourseUpdatedEvent(payload: {
        courseId: string;
    }): Promise<void> {
    ...
    async handleCourseDeletedEvent(payload: {
        courseId: string;
    }): Promise<void> {
