...
export class InvalidationService implements OnModuleInit {
  ...
  private async initializeInvalidationRules() {
    ...
    this.logger.log(
      `Initialized ${this.invalidationRules.size} invalidation rules`,
    );
  }
  ...
  private async scheduleInvalidation(
    ...
    if (rule.delay && rule.delay > 0) {
      ...
      this.logger.debug(
        `Scheduled delayed invalidation for rule ${rule.name} (${rule.delay}ms delay)`,
      );
    } else {
      // Execute immediately
      await this.executeInvalidation(rule, eventName, data, invalidationId);
  ...
  private async executeInvalidation(
    ...
    try {
      ...
      await Promise.all(
        keysToInvalidate.map((key) => this.caching.delete(key)),
      );
      ...
      event.success = true;
      this.analytics.recordCacheInvalidation(
        rule.id,
      ...
      this.logger.log(
        `Cache invalidation completed: ${rule.name} (${keysToInvalidate.length} keys invalidated)`,
      );
    } catch (error) {
      event.success = false;
    ...
    if (data && typeof data === 'object') {
      ...
      if (data.userId) {
        keys.push(
          ...
          `user:preferences:${data.userId}`,
          `user:permissions:${data.userId}`,
        );
      }
      ...
      if (data.contentId) {
        keys.push(
          ...
          `content:details:${data.contentId}`,
        );
      }
      ...
      if (data.categoryId) {
        keys.push(
          ...
          `category:content:${data.categoryId}`,
        );
      }
  ...
  private async cascadeInvalidation(
    ...
    if (cascadeKeys.length > 0) {
      ...
      this.logger.debug(
        `Cascade invalidation: ${cascadeKeys.length} additional keys invalidated`,
      );
    }
  }
  ...
  async invalidateByTags(
    ...
    try {
      ...
      event.error = error.message;
      this.logger.error(
        `Manual cache invalidation failed: ${tags.join(', ')}`,
        error,
      );
    }
  ...
  async invalidateKeys(
    ...
    try {
      ...
      event.success = true;
      this.logger.log(
        `Manual cache invalidation: ${keys.length} keys invalidated`,
      );
    } catch (error) {
      event.error = error.message;
  ...
  async updateRule(
    id: string,
    updates: Partial<InvalidationRule>,
  ): Promise<void> {
    const rule = this.invalidationRules.get(id);
    if (rule) {
  ...
  async getInvalidationStats(): Promise<any> {
    ...
    const recent24h = this.invalidationHistory.filter(
      (e) => e.timestamp >= last24Hours,
    );
    const recent7d = this.invalidationHistory.filter(
    ...
    return {
      ...
      activeRules: Array.from(this.invalidationRules.values()).filter(
        (r) => r.enabled,
      ).length,
    };
  }
