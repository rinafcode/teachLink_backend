...
export class CacheWarmingService implements OnModuleInit {
  ...
  private async initializeWarmingStrategies() {
    ...
    this.logger.log(
      `Initialized ${this.warmingStrategies.size} warming strategies`,
    );
  }
  ...
  async executeWarmingStrategy(strategy: WarmingStrategy): Promise<WarmingJob> {
    ...
    try {
      ...
      this.logger.debug(
        `Warming ${keysToWarm.length} keys for strategy ${strategy.name}`,
      );
      ...
      this.logger.log(
        `Cache warming completed: ${jobId} (${job.keysWarmed} keys warmed, ${job.errors.length} errors)`,
      );
    } catch (error) {
      job.status = 'failed';
      job.endTime = new Date();
  ...
  async updateStrategySchedule(name: string, schedule: string): Promise<void> {
    ...
    if (strategy) {
      ...
      this.logger.log(
        `Updated schedule for warming strategy ${name}: ${schedule}`,
      );
    }
  }
}
