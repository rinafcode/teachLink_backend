...
export class RedisClusterService implements OnModuleInit, OnModuleDestroy {
  ...
  private async connect() {
    try {
      const nodes = (process.env.REDIS_CLUSTER_NODES || "localhost:7000,localhost:7001,localhost:7002")
        .split(",")
        .map((node) => {
      ...
      this.cluster.on("node error", (error, node) => {
        this.logger.error(`Redis node error: ${node.options.host}:${node.options.port}`, error)
      })
