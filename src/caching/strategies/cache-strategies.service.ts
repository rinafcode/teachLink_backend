...
export class CacheStrategiesService {
    ...
    this.strategies.set('lfu', {
      ...
      processValue: async (key, value, options) => {
        ...
        return options.compress
          ? await this.compressValue(processedValue)
          : processedValue;
      },
      calculateTTL: (key, value, options) => {
  ...
  async applyStrategy(
    ...
    if (!strategy) {
      this.logger.warn(
        `Unknown cache strategy: ${strategyName}, using default`,
      );
      return value;
    }
    ...
    if (!strategy.shouldCache(key, value, options)) {
      throw new Error(
        `Strategy ${strategyName} determined value should not be cached`,
      );
    }
    ...
    try {
      ...
      this.logger.debug(
        `Applied ${strategyName} strategy to ${key} (TTL: ${ttl}s)`,
      );
      ...
      this.logger.error(
        `Error applying strategy ${strategyName} to ${key}`,
        error,
      );
      return value;
    }
