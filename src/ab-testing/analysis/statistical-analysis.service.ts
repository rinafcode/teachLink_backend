...
export class StatisticalAnalysisService {
     ...
     */
    async calculateStatisticalSignificance(experimentId: string): Promise<unknown> {
        ...
        if (controlVariant) {
            ...
            results.statisticallySignificant = await this.checkSignificanceAgainstControl(experiment.variants, controlMetrics, experiment.confidenceLevel);
        }
        return results;
    }
    /**
     ...
     */
    private async analyzeVariant(variant: ExperimentVariant, confidenceLevel: number): Promise<unknown> {
     ...
     */
    private async calculateMetricStatistics(metric: VariantMetric, confidenceLevel: number): Promise<unknown> {
     ...
     */
    private async checkSignificanceAgainstControl(variants: ExperimentVariant[], controlMetrics: VariantMetric[], confidenceLevel: number): Promise<boolean> {
        ...
        for (const variant of variants) {
            ...
            for (let i = 0; i < controlMetrics.length && i < variantMetrics.length; i++) {
                ...
                const isSignificant = await this.compareMetrics(controlMetric, variantMetric, confidenceLevel);
                if (isSignificant) {
                    return true;
                }
            }
     ...
     */
    private async compareMetrics(metric1: VariantMetric, metric2: VariantMetric, confidenceLevel: number): Promise<boolean> {
        // Calculate pooled standard error for comparison
        const pooledSE = Math.sqrt(Math.pow(metric1.standardDeviation || 0, 2) / (metric1.sampleSize || 1) +
     ...
     */
    async calculateEffectSize(experimentId: string): Promise<unknown> {
     ...
     */
    private async calculateCohensD(controlMetrics: VariantMetric[], variantMetrics: VariantMetric[]): Promise<number> {
        ...
        const controlStdDev = Math.sqrt(controlMetrics.reduce((sum, m) => sum + Math.pow(m.value - controlMean, 2), 0) /
            (controlMetrics.length - 1));
        const variantStdDev = Math.sqrt(variantMetrics.reduce((sum, m) => sum + Math.pow(m.value - variantMean, 2), 0) /
            (variantMetrics.length - 1));
        const pooledStdDev = Math.sqrt(((controlMetrics.length - 1) * Math.pow(controlStdDev, 2) +
