...
export class AutomatedDecisionService {
    ...
    private variantRepository: Repository<ExperimentVariant>, private statisticalAnalysisService: StatisticalAnalysisService) { }
    /**
     * Automatically selects winner for an experiment
     */
    async autoSelectWinner(experimentId: string, criteria?: WinnerSelectionCriteria): Promise<unknown> {
     ...
     */
    private async determineWinner(experiment: Experiment, statisticalResults: unknown, criteria: WinnerSelectionCriteria): Promise<ExperimentVariant | null> {
        ...
        for (const variantAnalysis of statisticalResults.variants) {
            ...
            const hasSufficientSample = variantAnalysis.metrics.every((metric: unknown) => metric.sampleSize >= criteria.minimumSampleSize);
            if (!hasSufficientSample)
                continue;
            // Check if statistically significant
            const isSignificant = variantAnalysis.metrics.some((metric: unknown) => metric.isStatisticallySignificant);
            if (!isSignificant)
                continue;
            // Check effect size
            const effectSize = await this.calculateEffectSizeForVariant(experiment.id, variant.id, controlVariant.id);
            if (effectSize < criteria.effectSizeThreshold)
                continue;
     ...
     */
    private async calculateEffectSizeForVariant(_experimentId: string, _variantId: string, _controlId: string): Promise<number> {
     ...
     */
    async getDecisionRecommendations(experimentId: string): Promise<unknown> {
     ...
     */
    private async calculateVariantPerformanceScores(variants: ExperimentVariant[]): Promise<unknown[]> {
