...
export class IndexingService implements OnModuleInit {
    ...
    async indexDocument(index: string, id: string, document: unknown): Promise<unknown> {
        return this.elasticsearchService.index({ index, id, document });
    }
    async updateDocument(index: string, id: string, document: unknown): Promise<unknown> {
        return this.elasticsearchService.update({ index, id, doc: document });
    }
    async deleteDocument(index: string, id: string): Promise<unknown> {
        return this.elasticsearchService.delete({ index, id });
    }
    async createIndex(index: string, mapping: unknown): Promise<unknown> {
        return this.elasticsearchService.indices.create({ index, mappings: mapping });
    }
    async bulkIndex(documents: unknown[]): Promise<unknown> {
    ...
    async syncCourse(course: Record<string, unknown>): Promise<void> {
    ...
    async updateCourse(id: string, fields: Record<string, unknown>): Promise<void> {
    ...
    async reindexAll(courses: Array<Record<string, unknown>>): Promise<unknown> {
        if (courses.length === 0)
            return;
        ...
        if (result.errors) {
            const failed = result.items.filter((item: unknown) => item.index?.error);
    ...
    async createCoursesIndex(forceReindex = false): Promise<unknown> {
    ...
    private async reindexCoursesIndexWithCurrentMapping(): Promise<void> {
        ...
        this.logger.log(`SEARCH_REINDEX_ON_BOOT enabled, reindexing ${COURSES_INDEX} using temporary index ${tempIndex}`);
        try {
    ...
    async createSearchAnalyticsIndex(): Promise<unknown> {
