...
@Injectable()
export class SearchAnalyticsEnhancedService {
  private readonly logger = new Logger(SearchAnalyticsEnhancedService.name);
  ...
  private async getZeroResultRate(timeRange: {
    ...
    const zeroResults = aggs7?.zero_results?.value || 0;
  ...
  private async getTimeDistribution(timeRange: {
    ...
    aggs5?.hourly_distribution?.buckets?.forEach((bucket: any) => {
  ...
  private async getResponseTimePercentiles(timeRange: {
    ...
    const aggs8 = result.aggregations as any;
    const percentiles = aggs8?.response_time_percentiles?.values || {};
    return {
  ...
  private async updateSearchEventWithClick(
    ...
    try {
      ...
      this.logger.error('Failed to update search event with click', error);
    }
  }
