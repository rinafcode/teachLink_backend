...
describe('SearchService', () => {
    ...
    beforeEach(() => {
        ...
        elasticsearchService.index.mockResolvedValue({} as unknown);
        cachingService = {
        ...
        service = new SearchService(elasticsearchService, autoCompleteService, searchFiltersService, cachingService);
    });
    it('sanitizes input, normalizes filters, and applies bounded pagination', async () => {
        ...
        const result = await service.performSearch('  NestJS   ', { category: ' Backend ', language: ['EN', 'en', ''], price: { gte: 0, invalid: 'x' } }, 'relevance', { page: 0, limit: 100 });
        expect(result.page).toBe(1);
        expect(result.limit).toBe(50);
        expect(elasticsearchService.search).toHaveBeenCalledTimes(1);
        const esQuery = elasticsearchService.search.mock.calls[0][0] as unknown;
        ...
        expect(esQuery.query.function_score.query.bool.filter).toEqual(expect.arrayContaining([
    ...
    it('normalizes autocomplete input before cache and downstream call', async () => {
        ...
        expect(cachingService.getOrSet).toHaveBeenCalledWith(expect.stringContaining('autocomplete:nestjs'), expect.any(Function), expect.any(Number));
