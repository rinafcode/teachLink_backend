...
export class VersionControlService {
     ...
     */
    async recordChange(sessionId: string, userId: string, change: unknown): Promise<ChangeRecord> {
     ...
     */
    private getOperationType(change: unknown): string {
        if (change.type) {
            return change.type;
        }
        if (change.operation) {
     ...
     */
    private getPreviousValue(history: VersionHistory, _change: unknown): unknown {
