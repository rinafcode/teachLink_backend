...
export class UsersService {
    ...
    private readonly userRepository: Repository<User>, private readonly cachingService: CachingService, private readonly eventEmitter: EventEmitter2, private readonly configService: ConfigService) { }
    ...
    async findAll(filter?: GetUsersDto): Promise<PaginatedResponse<User>> {
        ...
        return this.cachingService.getOrSet(cacheKey, async () => {
            ...
            if (filter?.status) {
                const status = enforceWhitelistedValue(filter.status, Object.values(UserStatus), 'status');
                query.andWhere('user.status = :status', { status });
            }
            if (filter?.search) {
                const safeSearch = sanitizeSqlLike(filter.search);
                // eslint-disable-next-line quotes
                query.andWhere("(user.email ILIKE :search ESCAPE '\\' OR user.firstName ILIKE :search ESCAPE '\\' OR user.lastName ILIKE :search ESCAPE '\\')", // eslint-disable-line quotes
                { search: `%${safeSearch}%` });
            }
    ...
    async updatePasswordResetToken(userId: string, token: string | null, expires: Date | null): Promise<void> {
        await this.userRepository.update(userId, {
            passwordResetToken: token as unknown as string,
            passwordResetExpires: expires as unknown as Date,
        });
    }
    async updateEmailVerificationToken(userId: string, token: string | null, expires: Date | null): Promise<void> {
        await this.userRepository.update(userId, {
