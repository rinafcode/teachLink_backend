...
export class AlertingService {
  ...
  async checkThresholds(metrics: PerformanceMetrics, analysis: PerformanceAnalysis): Promise<void> {
    // Check CPU usage
    ...
    const dbConnectionPercent = (metrics.database.activeConnections / 100) * 100
    await this.checkMetricThreshold(
  ...
  private async sendSmsAlert(alert: Alert, phoneNumber: string): Promise<void> {
    ...
    this.logger.debug("SMS message prepared", { to: phoneNumber, message: smsMessage })
  }
  ...
  async sendRegressionAlert(analysis: PerformanceAnalysis): Promise<void> {
    const alert = await this.createAlert({
      ...
      message: "Significant performance degradation detected compared to baseline",
  ...
  async getActiveAlerts(): Promise<Alert[]> {
    return this.activeAlerts.filter((alert) => !alert.acknowledged && !alert.resolvedAt)
  }
  ...
  async updateThreshold(metric: string, warning: number, critical: number): Promise<void> {
    const threshold = this.thresholds.find((t) => t.metric === metric)
    if (threshold) {
      threshold.warning = warning
      threshold.critical = critical
      this.logger.log(`Updated threshold for ${metric}: warning=${warning}, critical=${critical}`)
    }
  }
  ...
  async getAlertStatistics() {
    ...
    return {
      ...
      byType: {
        performance: this.alertHistory.filter((a) => a.type === "performance").length,
        error: this.alertHistory.filter((a) => a.type === "error").length,
        regression: this.alertHistory.filter((a) => a.type === "regression").length,
        threshold: this.alertHistory.filter((a) => a.type === "threshold").length,
      },
    }
  }
}
