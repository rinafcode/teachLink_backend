...
export class ScheduledTaskMonitoringService {
    ...
    startExecution(taskName: string, config: ScheduledTaskConfig, metadata: Record<string, unknown> = {}): string {
    ...
    recordRetry(taskName: string, attempt: number, maxRetries: number, reason?: string): void {
        ...
        this.alertingService.sendAlert('SCHEDULED_TASK_RETRY', `Task ${taskName} retry attempt ${attempt}/${maxRetries}${reason ? `: ${reason}` : ''}`, 'WARNING');
    }
    ...
    monitorScheduledTasks(): void {
        ...
        for (const [executionId, execution] of this.activeExecutions.entries()) {
            ...
            if (elapsedMs > config.timeoutMs) {
                ...
                this.activeExecutions.delete(executionId);
                this.alertingService.sendAlert('SCHEDULED_TASK_TIMEOUT', `Task ${execution.taskName} exceeded timeout (${elapsedMs}ms)`, 'CRITICAL');
            }
        }
        for (const [taskName, config] of this.taskConfigs.entries()) {
            ...
            const shouldAlertAgain = !alreadyAlertedAt || now.getTime() - alreadyAlertedAt.getTime() > config.expectedIntervalMs;
            if (isOverdue && shouldAlertAgain) {
                this.lastMissedAlertAt.set(taskName, now);
                this.alertingService.sendAlert('SCHEDULED_TASK_MISSED', `Task ${taskName} appears delayed/missed. Last start was ${baseline.toISOString()}`, 'CRITICAL');
            }
        }
    }
    getDashboard() {
        ...
        const tasks = Array.from(this.taskConfigs.entries()).map(([taskName, config]) => {
            ...
            const activeCount = Array.from(this.activeExecutions.values()).filter((entry) => entry.taskName === taskName).length;
            const threshold = config.expectedIntervalMs + (config.missedExecutionGraceMs || 0);
            const missed = !!lastExecution &&
        ...
        return {
            ...
            summary: {
                ...
                tasksWithRecentFailures: tasks.filter((task) => task.lastExecution?.status === 'FAILED' || task.lastExecution?.status === 'TIMED_OUT').length,
            },
            tasks,
        };
    }
