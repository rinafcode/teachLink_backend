...
export class MonitoringService implements OnModuleInit {
  ...
  async collectMetrics() {
    try {
      ...
      const analysis = await this.performanceAnalysis.analyzeMetrics(
        metrics,
        this.metricsHistory,
      );
      ...
      if (analysis.hasBottlenecks) {
        const recommendations = await this.optimization.generateRecommendations(
          metrics,
          analysis,
        );
  ...
  async performDeepAnalysis() {
    try {
      ...
      const deepAnalysis =
        await this.performanceAnalysis.performDeepAnalysis(recentMetrics);
      ...
      if (deepAnalysis.criticalIssues.length > 0) {
        await this.optimization.applyAutoOptimizations(
          deepAnalysis.criticalIssues,
        );
      }
    } catch (error) {
  ...
  async getPerformanceReport() {
    ...
    const analysis =
      await this.performanceAnalysis.generateReport(recentMetrics);
