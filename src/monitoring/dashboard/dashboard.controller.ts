...
export class DashboardController {
  ...
  async getOptimizations() {
    try {
      const recommendations =
        await this.optimization.getActiveRecommendations();
  ...
  async warmCache(@Body() body: { keys: string[] }) {
    try {
      ...
      return {
        success: true,
        message: `Warmed ${body.keys.length} cache keys`,
      };
    } catch (error) {
  ...
  private calculateMetricsSummary(metrics: any[]) {
    ...
    return {
      ...
      avgMemory:
        metrics.reduce(
          (sum, m) => sum + (m.memory.used / m.memory.total) * 100,
          0,
        ) / metrics.length,
      avgResponseTime:
        metrics.reduce((sum, m) => sum + m.http.avgResponseTime, 0) /
        metrics.length,
      avgErrorRate:
        metrics.reduce((sum, m) => sum + m.http.errorRate, 0) / metrics.length,
