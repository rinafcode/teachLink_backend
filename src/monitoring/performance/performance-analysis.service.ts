...
export class PerformanceAnalysisService {
  ...
  private detectBottlenecks(metrics: PerformanceMetrics): BottleneckInfo[] {
    ...
    if (memoryUsagePercent > this.thresholds.memory.warning) {
      bottlenecks.push({
        type: 'memory',
        severity:
          memoryUsagePercent > this.thresholds.memory.critical
            ? 'critical'
            : 'high',
    ...
    if (
      metrics.database.avgQueryTime > this.thresholds.avgResponseTime.warning
    ) {
      bottlenecks.push({
        type: 'database',
        severity:
          metrics.database.avgQueryTime >
          this.thresholds.avgResponseTime.critical
            ? 'critical'
            : 'high',
    ...
    if (
      metrics.http.avgResponseTime > this.thresholds.avgResponseTime.warning
    ) {
      bottlenecks.push({
        type: 'http',
        severity:
    ...
    if (metrics.eventLoop.delay > this.thresholds.eventLoopDelay.warning) {
      bottlenecks.push({
        type: 'eventloop',
        severity:
          metrics.eventLoop.delay > this.thresholds.eventLoopDelay.critical
            ? 'critical'
            : 'high',
  ...
  private analyzeTrends(history: PerformanceMetrics[]): PerformanceTrend[] {
    ...
    const previousCpuAvg =
      previous.reduce((sum, m) => sum + m.cpu.usage, 0) / previous.length;
    const cpuChange = ((recentCpuAvg - previousCpuAvg) / previousCpuAvg) * 100;

    trends.push({
      metric: 'CPU Usage',
      direction:
        cpuChange > 5 ? 'degrading' : cpuChange < -5 ? 'improving' : 'stable',
    ...
    const recentMemoryAvg =
      recent.reduce((sum, m) => sum + m.memory.used / m.memory.total, 0) /
      recent.length;
    const previousMemoryAvg =
      previous.reduce((sum, m) => sum + m.memory.used / m.memory.total, 0) /
      previous.length;
    const memoryChange =
      ((recentMemoryAvg - previousMemoryAvg) / previousMemoryAvg) * 100;
    ...
    const recentResponseAvg =
      recent.reduce((sum, m) => sum + m.http.avgResponseTime, 0) /
      recent.length;
    const previousResponseAvg =
      previous.reduce((sum, m) => sum + m.http.avgResponseTime, 0) /
      previous.length;
    const responseChange =
      ((recentResponseAvg - previousResponseAvg) / previousResponseAvg) * 100;
  ...
  private detectRegression(history: PerformanceMetrics[]): boolean {
    ...
    const recentAvgResponse =
      recent.reduce((sum, m) => sum + m.http.avgResponseTime, 0) /
      recent.length;
    const baselineAvgResponse =
      baseline.reduce((sum, m) => sum + m.http.avgResponseTime, 0) /
      baseline.length;

    const regressionThreshold = 0.5; // 50% increase
    return (
      (recentAvgResponse - baselineAvgResponse) / baselineAvgResponse >
      regressionThreshold
    );
  }
  ...
  async generateReport(metrics: PerformanceMetrics[]): Promise<any> {
    ...
    return {
      ...
      details: analysis,
      recommendations: analysis.criticalIssues.map(
