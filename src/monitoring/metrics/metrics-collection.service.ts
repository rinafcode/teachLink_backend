...
export class MetricsCollectionService {
  ...
  async collectAllMetrics(): Promise<PerformanceMetrics> {
    const [cpuMetrics, memoryMetrics, dbMetrics, httpMetrics] =
      await Promise.all([
    ...
    try {
      ...
      return {
        activeConnections: Number.parseInt(
          connectionCount[0]?.active_connections || '0',
        ),
  ...
  private async collectHttpMetrics() {
    ...
    this.httpMetrics.lastMinuteRequests =
      this.httpMetrics.lastMinuteRequests.filter(
