...
export class EmailQueueProcessor {
    ...
    private readonly recipientRepository: Repository<CampaignRecipient>, private readonly emailSenderService: EmailSenderService, private readonly segmentationService: SegmentationService, private readonly abTestingService: ABTestingService) { }
    ...
    async handleCampaignProcessing(job: Job<{
        campaignId: string;
        recipients: string[];
    }>) {
    ...
    async handleResumeCampaign(job: Job<{
        campaignId: string;
    }>) {
    ...
    private async processRecipients(campaign: Campaign, users: Array<{
        id: string;
        email: string;
    }>): Promise<void> {
        // Create recipient records
        const recipients = users.map((user) => this.recipientRepository.create({
