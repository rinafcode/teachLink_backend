...
export class EmailMarketingService {
    ...
    private readonly emailQueue: Queue, private readonly segmentationService: SegmentationService, private readonly templateService: TemplateManagementService, private readonly abTestingService: ABTestingService, private readonly analyticsService: EmailAnalyticsService) { }
    /**
    ...
    async scheduleCampaign(id: string, scheduleDto: ScheduleCampaignDto): Promise<Campaign> {
        ...
        await this.emailQueue.add(JOB_NAMES.SEND_CAMPAIGN, { campaignId: id }, { delay, jobId: `campaign-${id}` });
        return this.campaignRepository.save(campaign);
    }
    /**
    ...
    async sendCampaign(id: string): Promise<Campaign> {
        ...
        const recipients = await this.segmentationService.getUsersFromSegments(campaign.segmentIds || []);
