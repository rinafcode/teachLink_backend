...
export class EmailSenderService {
    ...
    constructor(private readonly configService: ConfigService, private readonly templateService: TemplateManagementService, private readonly analyticsService: EmailAnalyticsService) {
    ...
    async sendEmail(options: SendEmailOptions): Promise<SendEmailResult> {
        try {
            ...
            if (options.templateId) {
                const rendered = await this.templateService.renderTemplate(options.templateId, options.variables || {});
                html = rendered.html;
                text = rendered.text;
            ...
            if (options.campaignId && options.recipientId) {
                await this.analyticsService.recordEvent(options.campaignId, options.recipientId, EmailEventType.SENT, { messageId: result.messageId, variantId: options.variantId });
            }
            ...
            // Record bounce event
            if (options.campaignId && options.recipientId) {
                await this.analyticsService.recordEvent(options.campaignId, options.recipientId, EmailEventType.BOUNCED, { error: error.message });
            }
    ...
    async sendBulkEmails(recipients: Array<{
        ...
        id: string;
        variables?: Record<string, unknown>;
    }>, options: Omit<SendEmailOptions, 'to' | 'recipientId'>): Promise<{
        sent: number;
        failed: number;
    ...
    private wrapLinksForTracking(html: string, campaignId: string, recipientId: string): string {
        const baseUrl = this.configService.get('APP_URL', 'http://localhost:3000');
        return html.replace(/<a\s+([^>]*href=["'])([^"']+)(["'][^>]*)>/gi, (match, prefix, url, suffix) => {
