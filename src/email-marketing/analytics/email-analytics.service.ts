...
export class EmailAnalyticsService {
     ...
     */
    async recordEvent(campaignId: string, recipientId: string, eventType: EmailEventType, metadata?: Record<string, unknown>): Promise<EmailEvent> {
     ...
     */
    async getCampaignTimeSeries(campaignId: string, startDate: Date, endDate: Date): Promise<TimeSeriesData[]> {
        ...
        for (const event of events) {
            ...
            if (event.eventType === EmailEventType.OPENED)
                data.opens++;
    ...
    private async getBatchMetrics(campaignIds: string[]): Promise<Map<string, CampaignMetrics>> {
        ...
        for (const row of regularCounts) {
            ...
            if (row.eventType === EmailEventType.DELIVERED)
                metrics.delivered = count;
            if (row.eventType === EmailEventType.BOUNCED)
                metrics.bounced = count;
            if (row.eventType === EmailEventType.UNSUBSCRIBED)
        ...
        for (const row of uniqueCounts) {
            const metrics = getMetrics(row.campaignId);
            const count = parseInt(row.count, 10);
            if (row.eventType === EmailEventType.OPENED)
                metrics.opened = count;
            if (row.eventType === EmailEventType.CLICKED)
