...
export class AutomationService {
    ...
    async create(createAutomationDto: CreateAutomationDto): Promise<AutomationWorkflow> {
        ...
        if (createAutomationDto.triggers?.length) {
            const triggers = createAutomationDto.triggers.map((trigger) => this.triggerRepository.create({
                ...trigger,
        ...
        if (createAutomationDto.actions?.length) {
            const actions = createAutomationDto.actions.map((action, index) => this.actionRepository.create({
                ...action,
                workflowId: savedWorkflow.id,
                order: index,
            }));
    ...
    async update(id: string, updateAutomationDto: UpdateAutomationDto): Promise<AutomationWorkflow> {
        ...
        if (updateAutomationDto.triggers) {
            ...
            const triggers = updateAutomationDto.triggers.map((trigger) => this.triggerRepository.create({
                ...trigger,
                workflowId: id,
            }));
            await this.triggerRepository.save(triggers);
        ...
        if (updateAutomationDto.actions) {
            ...
            const actions = updateAutomationDto.actions.map((action, index) => this.actionRepository.create({
                ...action,
                workflowId: id,
                order: index,
            }));
    ...
    async handleCourseEnrollment(payload: {
        userId: string;
        courseId: string;
    }) {
    ...
    async handleCourseCompletion(payload: {
        userId: string;
        courseId: string;
    }) {
     ...
     */
    async handleUserInactivity(payload: {
        userId: string;
     ...
     */
    private async executeTriggeredWorkflows(triggerType: TriggerType, payload: Record<string, unknown>): Promise<void> {
     ...
     */
    private evaluateTriggerConditions(trigger: AutomationTrigger, payload: Record<string, unknown>): boolean {
     ...
     */
    private async executeWorkflowActions(workflow: AutomationWorkflow, payload: Record<string, unknown>): Promise<void> {
     ...
     */
    private async executeAction(action: AutomationAction, payload: Record<string, unknown>): Promise<void> {
        switch (action.type) {
            ...
            case ActionType.WAIT:
                await this.emailQueue.add(JOB_NAMES.CONTINUE_AUTOMATION, {
