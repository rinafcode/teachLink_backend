...
export class SegmentationService {
    ...
    async create(createSegmentDto: CreateSegmentDto): Promise<Segment> {
        ...
        if (createSegmentDto.rules?.length) {
            const rules = createSegmentDto.rules.map((rule, index) => this.ruleRepository.create({
                ...rule,
     ...
     */
    private formatValue(value: unknown, operator: SegmentRuleOperator): string {
        if (operator === SegmentRuleOperator.CONTAINS ||
            operator === SegmentRuleOperator.NOT_CONTAINS) {
            return `'%${value}%'`;
        }
