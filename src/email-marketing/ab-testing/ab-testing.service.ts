...
export class ABTestingService {
    ...
    async create(createABTestDto: CreateABTestDto): Promise<ABTest> {
        ...
        const variants = createABTestDto.variants.map((v, index) => this.variantRepository.create({
            ...v,
    ...
    async getTestResults(id: string): Promise<{
        ...
        const { isSignificant, winner, confidenceLevel } = this.calculateSignificance(variantStats, test.winnerCriteria);
        if (winner) {
    ...
    private calculateSignificance(variants: VariantStats[], criteria: string): {
        isSignificant: boolean;
        winner: string | null;
        confidenceLevel: number;
    } {
