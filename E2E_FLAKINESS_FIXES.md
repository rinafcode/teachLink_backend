...
it('should register user', async () => {
  const response = await request(app.getHttpServer()).post('/api/auth/register').send(userData);
...
```typescript
await retryHelper.withRetry(() => httpClient.get('/api/data'), { maxAttempts: 3, delayMs: 500 });
```

### 2. Wait for Conditions

```typescript
await retryHelper.waitFor(() => checkDatabaseReady(), {
  timeout: 10000,
...
```typescript
await retryHelper.waitForStable(() => getQueueLength(), {
