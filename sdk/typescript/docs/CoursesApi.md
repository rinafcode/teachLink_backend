...
```typescript
import { CoursesApi, Configuration, CourseRequest } from './api';
...
const { status, data } = await apiInstance.createCourse(courseRequest);
```
...
const { status, data } = await apiInstance.listCourses(page, limit);
```
